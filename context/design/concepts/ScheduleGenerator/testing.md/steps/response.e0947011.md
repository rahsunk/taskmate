---
timestamp: 'Fri Oct 17 2025 15:32:55 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251017_153255.f7f31187.md]]'
content_id: e0947011715a608093d0badfd982c2041d0a8e144b8d720f0139c6ef1777783d
---
...
Deno.test("Principle: Create, add, generate, edit, delete, regenerate schedule", async () => {
  ...
  try {
    ...
    let fetchedSchedule = await concept._getSchedule({ scheduleId: userAScheduleId });
    ...
    let events = await concept._getScheduleEvents({ schedule: userAScheduleId });
    ...
    let tasks = await concept._getScheduleTasks({ schedule: userAScheduleId });
    ...
    const genSchedule1 = await concept._getSchedule({ scheduleId: generatedSchedule1Id });
    ...
    events = await concept._getScheduleEvents({ schedule: userAScheduleId });
    ...
    tasks = await concept._getScheduleTasks({ schedule: userAScheduleId });
    ...
    const genSchedule2 = await concept._getSchedule({ scheduleId: generatedSchedule2Id });
...
Deno.test("Action: initializeSchedule creates a valid schedule", async () => {
  ...
  try {
    ...
    const newSchedule = await concept._getSchedule({ scheduleId: newScheduleId });
...
Deno.test("Action: addEvent requires an existing schedule", async () => {
  ...
  try {
    ...
    const { schedule: existingScheduleId } = (await concept.initializeSchedule({ owner: userA })) as { schedule: ID };
    ...
    const events = await concept._getScheduleEvents({ schedule: existingScheduleId });
...
Deno.test("Action: editEvent requirements and success", async () => {
  ...
  try {
    ...
    const { schedule: scheduleId } = (await concept.initializeSchedule({ owner: userA })) as { schedule: ID };
    const { event: eventId } = (await concept.addEvent({
      schedule: scheduleId,
      name: "Old Name",
      startTime: "08:00",
      endTime: "09:00",
    ...
    console.log(`Action: Attempting to edit event from another schedule ${anotherScheduleId}...`);
    result = await concept.editEvent({
      schedule: anotherScheduleId, // Incorrect schedule
      oldEvent: eventId,
      name: "New Name",
      startTime: "13:00",
      endTime: "14:00",
    ...
    assertEquals("error" in result, true, "Editing an event not belonging to the schedule should fail.");
    ...
    console.log("Result: Failed as expected (wrong schedule).");
    ...
    const updatedEvent = (await concept._getScheduleEvents({ schedule: scheduleId }) as any[]).find(e => e._id === eventId);
...
Deno.test("Action: deleteEvent requirements and success", async () => {
  ...
  try {
    ...
    const { schedule: scheduleId } = (await concept.initializeSchedule({ owner: userA })) as { schedule: ID };
    const { event: eventToDeleteId } = (await concept.addEvent({
      schedule: scheduleId,
    ...
    const { event: otherEventId } = (await concept.addEvent({
      schedule: scheduleId,
      name: "Other Event",
      startTime: "12:00",
      endTime: "13:00",
    ...
    console.log(`Action: Attempting to delete event from another schedule ${anotherScheduleId}...`);
    result = await concept.deleteEvent({ schedule: anotherScheduleId, event: eventToDeleteId }); // Event is not in this schedule
    assertEquals("error" in result, true, "Deleting an event not belonging to the schedule should fail.");
    assertEquals((result as { error: string }).error, `Schedule with ID '${anotherScheduleId}' not found.`, "Expected schedule not found error for wrong schedule ID.");
    console.log("Result: Failed as expected (wrong schedule).");
    ...
    const eventsAfterDelete = await concept._getScheduleEvents({ schedule: scheduleId });
...
Deno.test("Action: addTask requirements and success", async () => {
  ...
  try {
    ...
    assertEquals("error" in successResult, false, "Adding to an existing schedule should succeed.");
    const tasks = await concept._getScheduleTasks({ schedule: existingScheduleId });
...
Deno.test("Action: editTask requirements and success", async () => {
  ...
  try {
    ...
    const { schedule: scheduleId } = (await concept.initializeSchedule({ owner: userA })) as { schedule: ID };
    const { task: taskId } = (await concept.addTask({
      schedule: scheduleId,
      name: "Old Task Name",
      deadline: "2024-07-20",
    ...
    console.log(`Action: Attempting to edit task from another schedule ${anotherScheduleId}...`);
    ...
    assertEquals("error" in result, true, "Editing a task not belonging to the schedule should fail.");
    assertEquals((result as { error: string }).error, `Schedule with ID '${anotherScheduleId}' not found.`, "Expected schedule not found error for wrong schedule ID.");
    console.log("Result: Failed as expected (wrong schedule).");
    ...
    const updatedTask = (await concept._getScheduleTasks({ schedule: scheduleId }) as any[]).find(t => t._id === taskId);
...
Deno.test("Action: deleteTask requirements and success", async () => {
  ...
  try {
    ...
    const { schedule: scheduleId } = (await concept.initializeSchedule({ owner: userA })) as { schedule: ID };
    const { task: taskToDeleteId } = (await concept.addTask({
      schedule: scheduleId,
      name: "Task to Delete",
      deadline: "2024-07-20",
      expectedCompletionTime: 5,
    ...
    const { task: otherTaskId } = (await concept.addTask({
      schedule: scheduleId,
      name: "Other Task",
      deadline: "2024-07-25",
      expectedCompletionTime: 3,
    ...
    console.log(`Action: Attempting to delete task from another schedule ${anotherScheduleId}...`);
    result = await concept.deleteTask({ schedule: anotherScheduleId, task: taskToDeleteId }); // Task is not in this schedule
    assertEquals("error" in result, true, "Deleting a task not belonging to the schedule should fail.");
    assertEquals((result as { error: string }).error, `Schedule with ID '${anotherScheduleId}' not found.`, "Expected schedule not found error for wrong schedule ID.");
    console.log("Result: Failed as expected (wrong schedule).");
    ...
    const tasksAfterDelete = await concept._getScheduleTasks({ schedule: scheduleId });
...
Deno.test("Action: generateSchedule error conditions and success", async () => {
  ...
  try {
    ...
    const { schedule: userCScheduleId } = (await concept.initializeSchedule({ owner: "user:Charlie" as ID })) as { schedule: ID };
    await concept.addEvent({ schedule: userCScheduleId, name: "Single Event", startTime: "10:00", endTime: "11:00", repeatTime: "None" });
    await concept.addTask({ schedule: userCScheduleId, name: "Single Task", deadline: "2024-08-01", expectedCompletionTime: 1, priority: 50 });
    ...
    const generatedSchedule = await concept._getSchedule({ scheduleId: generatedScheduleId });
...
// trace:
// 1. initializeSchedule for userA -> userAScheduleId
// 2. addEvent(userAScheduleId, "Team Meeting", "09:00", "10:00", "Daily") -> event1Id
// 3. addEvent(userAScheduleId, "Project Review", "11:00", "12:00", "Weekly") -> event2Id
// 4. addTask(userAScheduleId, "Write Report", "2024-07-31", 4, 90) -> task1Id
// 5. addTask(userAScheduleId, "Prepare Presentation", "2024-08-05", 2, 70) -> task2Id
// 6. _getScheduleEvents(userAScheduleId) -> [event1Id, event2Id]
// 7. _getScheduleTasks(userAScheduleId) -> [task1Id, task2Id]
// 8. generateSchedule(userAScheduleId) -> generatedSchedule1Id (timestamp 1, events:[event1Id, event2Id], tasks:[task1Id, task2Id])
// 9. editEvent(userAScheduleId, event1Id, "Revised Team Meeting", "09:30", "10:30", "Daily")
// 10. deleteTask(userAScheduleId, task2Id)
// 11. _getScheduleEvents(userAScheduleId) -> [event1Id(modified), event2Id]
// 12. _getScheduleTasks(userAScheduleId) -> [task1Id]
// 13. generateSchedule(userAScheduleId) -> generatedSchedule2Id (timestamp 2, events:[event1Id, event2Id], tasks:[task1Id])
// 14. _getSchedule(generatedSchedule2Id) -> Verify contents and timestamp.
```
