---
timestamp: 'Mon Nov 03 2025 10:52:30 GMT-0500 (Eastern Standard Time)'
parent: '[[../20251103_105230.0fa908c6.md]]'
content_id: 4154c6b011eec9b8e303ed7a03432ba103fb41929c0c33e65a6231c2b6e99917
---

# response:

Here's the updated `ScheduleGeneratorConcept.ts` with the new query and the full `src/syncs/ScheduleGenerator.sync.ts` file, implementing synchronizations for all the listed actions and queries, including session-based authorization checks.

First, the updated `ScheduleGeneratorConcept.ts`:
...
Now, the `ScheduleGenerator.sync.ts` file:
...
const queryRequiresSession = async (
  ...
  if (!originalFrame || !originalFrame[sessionVar]) {
    return new Frames(); // No session in request or empty frame
  }

  let newFrames = await frames.query(
    Sessioning._getUser,
    { session: originalFrame[sessionVar] },
    { user: activeUserVar },
  );

  return newFrames; // Returns empty if session is invalid/not found
};

// Helper for checking session and schedule ownership
// Assumes that scheduleIdVar is bound in the incoming Requesting.request frame.
const authorizeScheduleAccess = async (
  ...
  const originalFrame = currentFrames.at(0);
  if (!originalFrame || !originalFrame[sessionVar] || !originalFrame[scheduleIdVar]) {
    return new Frames(); // Missing required parameters
  }

  // 1. Get the active user from the session
  currentFrames = await queryRequiresSession(frames, sessionVar, activeUserVar);
  if (currentFrames.length === 0) {
    return new Frames(); // Invalid or expired session
  }
  ...
  currentFrames = await currentFrames.query(
    ...
    { schedule: originalFrame[scheduleIdVar] as ID },
    { scheduleDetails: symbol("scheduleDetails") }, // Bind the result array to 'scheduleDetails'
  );

  if (currentFrames.length === 0 || currentFrames.at(0)![symbol("scheduleDetails")].length === 0) {
    return new Frames(); // Schedule not found
  }

  // 3. Filter to ensure the activeUser (from session) is the owner of the schedule
  return currentFrames.filter(($) => {
    const scheduleDetails = $[symbol("scheduleDetails")]; // This will be an array
    return scheduleDetails.length > 0 && scheduleDetails[0].owner === $[activeUserVar];
  });
};
...
const authorizeIndividualItemAccess = async (
  ...
  itemDetailsQuery: (args: { [k: string]: ID }) => Promise<{ [k: string]: unknown }[]>, // e.g., ScheduleGenerator._getEventDetails
  activeUserVar: symbol,
): Promise<Frames> => {
  let currentFrames = frames;
  const originalFrame = currentFrames.at(0);
  if (!originalFrame || !originalFrame[sessionVar] || !originalFrame[itemIdVar]) {
    return new Frames(); // Missing required parameters
  }
  ...
  currentFrames = await queryRequiresSession(frames, sessionVar, activeUserVar);
  if (currentFrames.length === 0) {
    return new Frames(); // Invalid session
  }

  // 2. Get item details (event or task) to find its scheduleID
  const itemId = originalFrame[itemIdVar] as ID;
  const itemDetailsResult = await itemDetailsQuery({ [itemIdVar]: itemId }); // Call the concept query directly
  if (itemDetailsResult.length === 0) {
    return new Frames(); // Item not found
  }

  // itemDetailsResult is an array like [{ itemDetails: { scheduleID: N, ... } }]
  const itemInternalScheduleID = (itemDetailsResult[0] as { itemDetails: { scheduleID: number } }).itemDetails.scheduleID;

  // Now, add the item's details and the internal schedule ID to the frames for subsequent filtering.
  // We need to re-create frames with the existing bindings + new ones
  const frameWithItemDetails = new Frames(
    currentFrames.map((f) => ({ ...f, [symbol("itemInternalScheduleID")]: itemInternalScheduleID })),
  );

  // 3. Get schedule details using the internal schedule ID
  let framesWithScheduleDetails = await frameWithItemDetails.query(
    ScheduleGenerator._getScheduleByInternalScheduleID,
    { internalScheduleID: symbol("itemInternalScheduleID") },
    { scheduleDetails: symbol("scheduleDetails") }, // Bind the result array
  );

  if (framesWithScheduleDetails.length === 0 || framesWithScheduleDetails.at(0)![symbol("scheduleDetails")].length === 0) {
    return new Frames(); // Schedule not found for item
  }

  // 4. Authorize ownership: active user must be the owner of the schedule
  return framesWithScheduleDetails.filter(($) => {
    const scheduleInfo = $[symbol("scheduleDetails")]; // This is an array
    return scheduleInfo.length > 0 && scheduleInfo[0].owner === $[activeUserVar];
  });
};


// Helper function for queries that retrieve all items (full docs) for the active user
// This aggregates items from all schedules owned by the active user.
const getAllItemDocsForActiveUser = async (
  frames: Frames,
  sessionVar: symbol,
  activeUserVar: symbol,
  itemCollection:
    | ScheduleGenerator["events"]
    | ScheduleGenerator["tasks"], // The actual collection object from the concept instance
  resultsVar: symbol, // e.g., symbol("event") or symbol("task") - the key in the final response
): Promise<Frames> => {
  let currentFrames = frames;
  const originalFrame = currentFrames.at(0);
  if (!originalFrame || !originalFrame[sessionVar]) {
    return new Frames({ ...originalFrame, [resultsVar]: [] }); // Invalid session, return empty results
  }
  ...
  const internalScheduleIDsResult = await ScheduleGenerator._getInternalScheduleIDsByOwner({ owner: activeUserId as ID });
  if (internalScheduleIDsResult.error || !internalScheduleIDsResult.internalScheduleID) {
    return new Frames({ ...originalFrame, [resultsVar]: [] }); // User has no schedules or error
  }
  ...
  return new Frames({ ...originalFrame, [resultsVar]: allItemDocs });
};
...
export const InitializeScheduleRequest: Sync = ({ request, session, owner, activeUser }) => ({
  ...
  where: async (frames) => {
    ...
    return frames.filter(($) => $[activeUser] === $[owner]);
  },
...
export const AddEventRequest: Sync = (
  ...
  where: (frames) => authorizeScheduleAccess(frames, session, scheduleId, activeUser),
  then: actions([
...
// _getScheduleByOwner (This query explicitly takes an 'owner' parameter, so we authorize by matching activeUser with requested owner)
export const GetScheduleByOwnerRequest: Sync = ({ request, session, owner, activeUser, schedule: scheduleId, error }) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    frames = await queryRequiresSession(frames, session, activeUser);
    if (frames.length === 0) {
      return new Frames({ ...frames.at(0), error: "Session invalid or expired." });
    }
    // Authorization: only allow a user to get their own schedule by owner ID
    frames = frames.filter(($) => $[activeUser] === $[owner]);
    if (frames.length === 0) {
      return new Frames({ ...frames.at(0), error: "Unauthorized: Cannot retrieve schedule for other users." });
    }
    // If authorized, perform the query
    frames = await frames.query(ScheduleGenerator._getScheduleByOwner, { owner }, { schedule: scheduleId, error });
    // Handle case where no schedule is found (query returns empty array in success case)
    if (frames.length === 0) {
        return new Frames({ ...frames.at(0), schedule: null }); // Return null schedule instead of no response
    }
    return frames;
  },
  then: actions([Requesting.respond, { request, schedule: scheduleId, error }]), // Respond with schedule or error
});

// _getEventsForSchedule
export const GetEventsForScheduleRequest: Sync = (
  { request, session, schedule: scheduleId, activeUser, event: events, error },
) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    if (frames.length === 0) {
      return new Frames({ ...frames.at(0), error: "Unauthorized or schedule not found." });
    }
    frames = await frames.query(ScheduleGenerator._getEventsForSchedule, { schedule: scheduleId }, { event: events, error });
    if (frames.length === 0) { // If query returns no events, ensure we still respond
      return new Frames({ ...frames.at(0), event: [], error: error });
    }
    return frames;
  },
  then: actions([Requesting.respond, { request, event: events, error }]),
});

// _getTasksForSchedule
export const GetTasksForScheduleRequest: Sync = (
  { request, session, schedule: scheduleId, activeUser, task: tasks, error },
) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(ScheduleGenerator._getTasksForSchedule, { schedule: scheduleId }, { task: tasks, error });
    if (frames.length === 0) { // If query returns no tasks, ensure we still respond
      return new Frames({ ...frames.at(0), task: [], error: error });
    }
    return frames;
  },
  then: actions([Requesting.respond, { request, task: tasks, error }]),
});

// _getEventDetails
export const GetEventDetailsRequest: Sync = (
  { request, session, event: eventId, activeUser, eventDetails, error },
) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    if (frames.length === 0) {
      return new Frames({ ...frames.at(0), error: "Unauthorized, event not found, or schedule not found." });
    }
    frames = await frames.query(ScheduleGenerator._getEventDetails, { event: eventId }, { eventDetails, error });
    if (frames.length === 0) { // If query returns no details, ensure we still respond
      return new Frames({ ...frames.at(0), eventDetails: null, error: error });
    }
    return frames;
  },
...
export const GetTaskDetailsRequest: Sync = (
  { request, session, task: taskId, activeUser, taskDetails, error },
) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    if (frames.length === 0) {
      return new Frames({ ...frames.at(0), error: "Unauthorized, task not found, or schedule not found." });
    }
    frames = await frames.query(ScheduleGenerator._getTaskDetails, { task: taskId }, { taskDetails, error });
    if (frames.length === 0) { // If query returns no details, ensure we still respond
      return new Frames({ ...frames.at(0), taskDetails: null, error: error });
    }
    return frames;
  },
...
export const GetAllSchedulesRequest: Sync = ({ request, session, activeUser, schedule: schedules, error }) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    // 2. Query all schedules owned by the active user (using _getScheduleByOwner which returns _id)
    let scheduleIdsResult = await ScheduleGenerator._getScheduleByOwner({ owner: activeUserId as ID });
    if (scheduleIdsResult.error) {
      return new Frames({ ...frames.at(0), error: scheduleIdsResult.error });
    }
    const scheduleIds = scheduleIdsResult.schedule ? [scheduleIdsResult.schedule] : []; // _getScheduleByOwner returns single ID or error

    if (scheduleIds.length === 0) {
      // If user has no schedules, return an empty array
      return new Frames({ ...frames.at(0), schedule: [] });
    }

    // 3. For each schedule ID, get full details
    const allScheduleDocs: any[] = []; // Collect full docs
    for (const sId of scheduleIds) {
      const detailsResult = await ScheduleGenerator._getScheduleDetails({ schedule: sId });
      if (detailsResult.scheduleDetails && detailsResult.scheduleDetails.length > 0) {
        allScheduleDocs.push(detailsResult.scheduleDetails[0]);
      }
    }

    // Return a single frame with the collected schedule documents
    return new Frames({ ...frames.at(0), [schedules]: allScheduleDocs });
  },
  then: actions([Requesting.respond, { request, schedule: schedules }]),
});


// _getScheduleDetails
export const GetScheduleDetailsRequest: Sync = (
  { request, session, schedule: scheduleId, activeUser, scheduleDetails, error },
) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(ScheduleGenerator._getScheduleDetails, { schedule: scheduleId }, { scheduleDetails, error });
    if (frames.length === 0) { // If query returns no details, ensure we still respond
      return new Frames({ ...frames.at(0), scheduleDetails: null, error: error });
    }
    return frames;
  },
...
export const GetAllEventsRequest: Sync = ({ request, session, activeUser, event: events, error }) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    frames = await getAllItemDocsForActiveUser(frames, session, activeUser, ScheduleGenerator.events, event);
    if (frames.length === 0) {
        return new Frames({ ...originalFrame, [event]: [], error: "Session invalid or expired, or no events found." });
    }
    // Check for explicit error from the helper
    if (frames.at(0)![error]) {
      return new Frames({ ...originalFrame, error: frames.at(0)![error] });
    }
    return frames;
  },
  then: actions([Requesting.respond, { request, event: events, error }]),
...
export const GetAllTasksRequest: Sync = ({ request, session, activeUser, task: tasks, error }) => ({
  when: actions([
    Requesting.request,
  ...
  where: async (frames) => {
    ...
    frames = await getAllItemDocsForActiveUser(frames, session, activeUser, ScheduleGenerator.tasks, task);
    if (frames.length === 0) {
        return new Frames({ ...originalFrame, [task]: [], error: "Session invalid or expired, or no tasks found." });
    }
    // Check for explicit error from the helper
    if (frames.at(0)![error]) {
      return new Frames({ ...originalFrame, error: frames.at(0)![error] });
