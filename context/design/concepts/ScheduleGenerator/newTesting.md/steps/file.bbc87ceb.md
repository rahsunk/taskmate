...
content_id: bbc87ceb0d6f8bd37c6fdd27599a14699845a94970cc44b3b3506c1ab57fa393
---

# file: src/ScheduleGenerator/ScheduleGeneratorConcept.test.ts

```typescript
import { assertEquals, assertExists, assertInstanceOf, assertNotEquals, assertArrayIncludes, assert } from "jsr:@std/assert";
...
// Helper for checking action results (single object return)
...
// Helper for checking query results (array of objects return)
const isQueryError = (result: any): result is Array<{ error: string }> => {
...
Deno.test("ScheduleGenerator Concept", async (t) => {
  ...
  const owner3: ID = generateTestId("user"); // For complex scheduling test
  ...
  await t.step("should fulfill the operational principle by generating an optimal schedule", async () => {
    ...
    assert(!isActionError(initScheduleResult), `Expected no error, got: ${initScheduleResult.error}`);
    ...
    const scheduleByOwnerResult = await concept._getScheduleByOwner({ owner: owner1 });
    assert(!isQueryError(scheduleByOwnerResult), `Expected no error, got: ${scheduleByOwnerResult[0]?.error}`);
    ...
    const scheduleDetailsResult = await concept._getScheduleDetails({ schedule: scheduleId });
    assert(!isQueryError(scheduleDetailsResult), `Expected no error, got: ${scheduleDetailsResult[0]?.error}`);
    const scheduleDetails = scheduleDetailsResult[0]?.scheduleDetails;
    ...
    assert(!isActionError(addEventResult), `Expected no error, got: ${addEventResult.error}`);
    ...
    assert(!isActionError(addTask1Result), `Expected no error, got: ${addTask1Result.error}`);
    ...
    assert(!isActionError(addTask2Result), `Expected no error, got: ${addTask2Result.error}`);
    ...
    assert(!isActionError(generateResult), `Expected no error, got: ${generateResult.error}`);
  ...
  await t.step("should initialize schedule and handle duplicate owner attempts", async () => {
    ...
    assert(!isActionError(initScheduleResult1), `Expected no error, got: ${initScheduleResult1.error}`);
    ...
    assert(!isQueryError(queryResult), `Expected no error, got: ${queryResult[0]?.error}`);
    ...
    assert(!isQueryError(allSchedulesResult), `Expected no error, got: ${allSchedulesResult[0]?.error}`);
    const allSchedules = allSchedulesResult.map(item => item.schedule);
    assertExists(allSchedules);
    ...
    assert(isQueryError(nonExistentScheduleDetails), "Should return an error for non-existent schedule details");
  ...
  await t.step("should manage events with valid and invalid operations", async () => {
    ...
    assert(!isActionError(addEventResult1), `Expected no error, got: ${addEventResult1.error}`);
    ...
    const event1DetailsResult = await concept._getEventDetails({ event: eventId1 });
    assert(!isQueryError(event1DetailsResult), `Expected no error, got: ${event1DetailsResult[0]?.error}`);
    ...
    assert(!isActionError(editEventResult), `Expected no error, got: ${editEventResult.error}`);
    console.log(`Event ${eventId1} edited.`);

    // Verify edited details
    const editedEvent1DetailsResult = await concept._getEventDetails({ event: eventId1 });
    assert(!isQueryError(editedEvent1DetailsResult), `Expected no error, got: ${editedEvent1DetailsResult[0]?.error}`);
    ...
    assert(!isActionError(deleteEventResult), `Expected no error, got: ${deleteEventResult.error}`);
    ...
    assert(isQueryError(deletedEventDetails), "Should return error for deleted event details");
    ...
    const eventsForScheduleResult = await concept._getEventsForSchedule({ schedule: scheduleId });
    assert(!isQueryError(eventsForScheduleResult), `Expected no error, got: ${eventsForScheduleResult[0]?.error}`);
    ...
    assert(!isQueryError(allEventsResult), `Expected no error, got: ${allEventsResult[0]?.error}`);
    assert(!allEventsResult.some(e => e.event?._id === eventId1), "Deleted event should not appear in all events.");
  ...
  await t.step("should manage tasks with valid and invalid operations", async () => {
    ...
    assert(!isActionError(addTaskResult1), `Expected no error, got: ${addTaskResult1.error}`);
    ...
    const task1DetailsResult = await concept._getTaskDetails({ task: taskId1 });
    assert(!isQueryError(task1DetailsResult), `Expected no error, got: ${task1DetailsResult[0]?.error}`);
    ...
    assert(!isActionError(editTaskResult), `Expected no error, got: ${editTaskResult.error}`);
    console.log(`Task ${taskId1} edited.`);

    // Verify edited details
    const editedTask1DetailsResult = await concept._getTaskDetails({ task: taskId1 });
    assert(!isQueryError(editedTask1DetailsResult), `Expected no error, got: ${editedTask1DetailsResult[0]?.error}`);
    ...
    assert(!isActionError(deleteTaskResult), `Expected no error, got: ${deleteTaskResult.error}`);
    ...
    assert(isQueryError(deletedTaskDetails), "Should return error for deleted task details");
    ...
    const tasksForScheduleResult = await concept._getTasksForSchedule({ schedule: scheduleId });
    assert(!isQueryError(tasksForScheduleResult), `Expected no error, got: ${tasksForScheduleResult[0]?.error}`);
    ...
    assert(!isQueryError(allTasksResult), `Expected no error, got: ${allTasksResult[0]?.error}`);
    assert(!allTasksResult.some(t => t.task?._id === taskId1), "Deleted task should not appear in all tasks.");
  ...
  await t.step("should correctly retrieve data using all query methods", async () => {
    ...
    assert(!isActionError(addTaskResult), `Expected no error, got: ${addTaskResult.error}`);
    assertExists(addTaskResult.task);
    const taskId = addTaskResult.task!;
    console.log(`Task 'Query Task' added: ${taskId}`);

    // _getScheduleByOwner
    const scheduleByOwnerResult = await concept._getScheduleByOwner({ owner });
    assert(!isQueryError(scheduleByOwnerResult), `Expected no error, got: ${scheduleByOwnerResult[0]?.error}`);
    const scheduleByOwner = scheduleByOwnerResult[0]?.schedule;
    assertExists(scheduleByOwner);
    assertEquals(scheduleByOwner, scheduleId);
    ...
    assertExists(eventsForSchedule);
    ...
    // _getEventDetails
    const eventDetailsResult = await concept._getEventDetails({ event: eventId });
    assert(!isQueryError(eventDetailsResult), `Expected no error, got: ${eventDetailsResult[0]?.error}`);
    ...
    // _getTaskDetails
    const taskDetailsResult = await concept._getTaskDetails({ task: taskId });
    assert(!isQueryError(taskDetailsResult), `Expected no error, got: ${taskDetailsResult[0]?.error}`);
    ...
    // _getScheduleDetails
    const specificScheduleDetailsResult = await concept._getScheduleDetails({ schedule: scheduleId });
    assert(!isQueryError(specificScheduleDetailsResult), `Expected no error, got: ${specificScheduleDetailsResult[0]?.error}`);
