...
content_id: f7c9e811033e03fefc52ee0bdfc44794614f02445b2225cf4cf682fb6ff36c29
---
...
export default class ScheduleGeneratorConcept {
   ...
   * @returns {Promise<Array<{schedule: Schedule} | {error: string}>>} - An array containing the schedule ID or an error message.
   */
  async _getScheduleByOwner({ owner }: { owner: User }): Promise<
    Array<{ schedule: Schedule } | { error: string }>
  > {
   ...
   * @returns {Promise<Array<{event: Event} | {error: string}>>} - An array of event IDs or an error message.
   */
  async _getEventsForSchedule({ schedule }: { schedule: Schedule }): Promise<
    Array<{ event: Event } | { error: string }>
  > {
    const existingSchedule = await this.schedules.findOne({ _id: schedule });
    if (!existingSchedule) {
      return [{ error: `Schedule with ID ${schedule} not found.` }];
    ...
    const eventDocs = await this.events
      ...
      .toArray();
    return eventDocs.map((doc) => ({ event: doc._id }));
  }
   ...
   * @returns {Promise<Array<{task: Task} | {error: string}>>} - An array of task IDs or an error message.
   */
  async _getTasksForSchedule({ schedule }: { schedule: Schedule }): Promise<
    Array<{ task: Task } | { error: string }>
  > {
    const existingSchedule = await this.schedules.findOne({ _id: schedule });
    if (!existingSchedule) {
      return [{ error: `Schedule with ID ${schedule} not found.` }];
    ...
    const taskDocs = await this.tasks
      .find({ scheduleID: existingSchedule.scheduleID })
      .toArray();
    return taskDocs.map((doc) => ({ task: doc._id }));
  }
   ...
   * @returns {Promise<Array<{eventDetails: EventDoc} | {error: string}>>} - An array containing the event document or an error message.
   */
  async _getEventDetails({ event }: { event: Event }): Promise<
    Array<{ eventDetails: EventDoc } | { error: string }>
  > {
   ...
   * @returns {Promise<Array<{taskDetails: TaskDoc} | {error: string}>>} - An array containing the task document or an error message.
   */
  async _getTaskDetails({ task }: { task: Task }): Promise<
    Array<{ taskDetails: TaskDoc } | { error: string }>
  > {
   ...
   *
   * @returns {Promise<Array<{schedule: ScheduleDoc} | {error: string}>>} - An array of all schedule documents or an error.
   */
  async _getAllSchedules(): Promise<
    Array<{ schedule: ScheduleDoc } | { error: string }>
  > {
    try {
   ...
   * @returns {Promise<Array<{scheduleDetails: ScheduleDoc} | {error: string}>>} - An array containing the ScheduleDoc object or an error.
   */
  async _getScheduleDetails({ schedule }: { schedule: Schedule }): Promise<
    Array<{ scheduleDetails: ScheduleDoc } | { error: string }>
  > {
    try {
   ...
   *
   * @returns {Promise<Array<{event: EventDoc} | {error: string}>>} - An array of all event documents or an error.
   */
  async _getAllEvents(): Promise<
    Array<{ event: EventDoc } | { error: string }>
  > {
    try {
   ...
   *
   * @returns {Promise<Array<{task: TaskDoc} | {error: string}>>} - An array of all task documents or an error.
   */
  async _getAllTasks(): Promise<
    Array<{ task: TaskDoc } | { error: string }>
  > {
    try {
