...
content_id: e5558f8eb9e6e25dd7b8567fe86f063b970ae8374561eeee2d81dba817d7cdcc
---
...
import { ID, Empty } from "@utils/types.ts"; // Import Empty type
...
Deno.test("Notification Concept Scenarios", async (t) => {
  ...
  let notificationConcept: NotificationConcept;

  // Setup hook to run before each test step
  Deno.test.beforeEach(async () => {
    // Get a fresh database and client for each test.
    // The `testDb` utility ensures the database is clean (dropped via Deno.test.beforeAll).
    [db, client] = await testDb();
    notificationConcept = new NotificationConcept(db);
    console.log(`\n--- Test Setup: New NotificationConcept instance for test "${t.name}" ---`);
  });

  // Teardown hook to run after each test step
  Deno.test.afterEach(async () => {
    // Close the MongoDB client to release resources.
    await client.close();
    console.log(`--- Test Teardown: DB client closed for test "${t.name}" ---`);
  });

  // --- Operational Principle Test ---
  // Demonstrates the core flow of the Notification concept: configure, send (simulated), and log a notification.
  await t.step("Operational Principle: Configure, Send, and Log Notification", async () => {
    ...
    // Creates a new notification configuration for user Alice for a specific post.
    ...
    let setting: NotificationSetting; // Declare setting variable outside for assignment within type guard
    if ("error" in createResult) {
      assertFalse(true, `Failed to create notification config: ${createResult.error}`);
      return; // Fail test if an unexpected error occurs
    } else {
      setting = createResult.setting; // Assign inside the narrowed success block
    }
    assertExists(setting, "Expected setting to be created."); // Now assertExists on the narrowed 'setting'
    ...
    // Triggers sending a notification based on the created configuration, using context data for templating.
    ...
    let sentNotif: SentNotification; // Declare sentNotif variable outside
    if ("error" in sendResult) {
      assertFalse(true, `Failed to send notification: ${sendResult.error}`);
      return; // Fail test if an unexpected error occurs
    } else {
      sentNotif = sendResult.sentNotification; // Assign inside the narrowed success block
    }
    assertExists(sentNotif, "Expected notification to be sent and logged."); // Assert on the narrowed 'sentNotif'
    ...
    // Verifies that the sent notification was logged persistently in the database.
  ...
  // --- Interesting Scenario 1: Edit Configuration ---
  ...
  await t.step("Scenario: Deleting a notification configuration and associated logs", async () => {
    ...
    let sentNotifID: ID; // Declare outside
    if ("error" in sendRes) {
      assertFalse(true, `Failed to send notification for deletion test: ${sendRes.error}`);
      return;
    } else {
      sentNotifID = sendRes.sentNotification._id; // Assign inside
    }
  ...
  // --- Interesting Scenario 3: Sending with Disabled/Non-Existent Config ---
  await t.step("Scenario: Sending notification with disabled or non-existent configuration", async () => {
    console.log(`\n--- Scenario: Sending with Disabled/Non-Existent Config ---`);
    ...
    if (!("error" in sendNoConfigRes)) { // Check for error first
      assertFalse(true, `Expected error when no config exists, but got success: ${JSON.stringify(sendNoConfigRes)}`);
      return;
    }
    assertExists(sendNoConfigRes.error, "Expected error message for no config."); // Assert error message exists on the narrowed type
    ...
    console.log("Output:", createRes);
    let disabledSettingID: ID; // Declare outside
    if ("error" in createRes) {
      assertFalse(true, `Failed to create disabled notification config: ${createRes.error}`);
      return;
    } else {
      disabledSettingID = createRes.setting._id; // Assign inside
    }
    ...
    if (!("error" in sendDisabledRes)) { // Check for error first
      assertFalse(true, `Expected error when config is disabled, but got success: ${JSON.stringify(sendDisabledRes)}`);
      return;
    }
    assertExists(sendDisabledRes.error, "Expected error message for disabled config."); // Assert error message exists on the narrowed type
  ...
  // --- Interesting Scenario 4: Error handling and Preconditions ---
  await t.step("Scenario: Precondition failures and error returns for actions", async () => {
    console.log(`\n--- Scenario: Precondition Failures ---`);
    ...
    if (!("error" in createMissingOwner)) { // Check for error first
      assertFalse(true, `Expected error for missing owner, but got success: ${JSON.stringify(createMissingOwner)}`);
      return;
    }
    assertExists(createMissingOwner.error, "Expected error for missing owner (empty string)."); // Assert error message on narrowed type
    ...
    if (!("error" in createInvalidTime)) { // Check for error first
      assertFalse(true, `Expected error for invalid dailyTime format, but got success: ${JSON.stringify(createInvalidTime)}`);
      return;
    }
    assertExists(createInvalidTime.error, "Expected error for invalid dailyTime format."); // Assert error message on narrowed type
    ...
    if (!("error" in createInvalidEmail)) { // Check for error first
      assertFalse(true, `Expected error for invalid email format, but got success: ${JSON.stringify(createInvalidEmail)}`);
      return;
    }
    assertExists(createInvalidEmail.error, "Expected error for invalid email format."); // Assert error message on narrowed type
    ...
    if (!("error" in editNonExistent)) { // Check for error first
      assertFalse(true, `Expected error for non-existent setting ID during edit, but got success: ${JSON.stringify(editNonExistent)}`);
      return;
    }
    assertExists(editNonExistent.error, "Expected error for non-existent setting ID during edit."); // Assert error message on narrowed type
    ...
    if (!("error" in deleteNonExistent)) { // Check for error first
    ...
    assertExists(deleteNonExistent.error, "Expected error for non-existent setting ID during delete."); // Assert error message on narrowed type
  ...
  await t.step("Scenario: Multiple notification settings for different items/types for one user", async () => {
    ...
    // This was already fixed in the previous iteration, no destructuring needed here.
  ...
  // --- Query Tests (Explicitly defined queries for verification) ---
  await t.step("Scenario: Querying functionality for notification settings and logs", async () => {
    ...
    if ("error" in s1Res) { assertFalse(true, `Failed to create s1: ${s1Res.error}`); return; } else { s1Setting = s1Res.setting; }
    if ("error" in s2Res) { assertFalse(true, `Failed to create s2: ${s2Res.error}`); return; } else { s2Setting = s2Res.setting; }
    if ("error" in s3Res) { assertFalse(true, `Failed to create s3: ${s3Res.error}`); return; } else { s3Setting = s3Res.setting; }
