...
content_id: b0db99e3f3471f6be55395c0c06349d9fc0e2a90e817e1dd637106696a312daf
---
...
Deno.test("Notification Concept Scenarios", async (t) => {
  ...
  await t.step("Operational Principle: Configure, Send, and Log Notification", async () => {
    ...
    // Type is now narrowed to { setting: NotificationSetting }
    const setting: NotificationSetting = createResult.setting;
    assertExists(setting, "Expected setting to be created (after type narrowing).");
    ...
    // Type is now narrowed to { sentNotification: SentNotification }
    const sentNotif: SentNotification = sendResult.sentNotification;
    assertExists(sentNotif, "Expected notification to be sent and logged (after type narrowing).");
  ...
  await t.step("Scenario: Sending notification with disabled or non-existent configuration", async () => {
    ...
    // Type is now narrowed to { error: string }
  ...
  await t.step("Scenario: Multiple notification settings for different items/types for one user", async () => {
    ...
    // Fix: Remove destructuring, directly assign the array
  ...
  await t.step("Scenario: Querying functionality for notification settings and logs", async () => {
    ...
    if ("error" in s1Res) assertFalse(true, `Failed to create s1: ${s1Res.error}`);
    if ("error" in s2Res) assertFalse(true, `Failed to create s2: ${s2Res.error}`);
    if ("error" in s3Res) assertFalse(true, `Failed to create s3: ${s3Res.error}`);
