...
content_id: 96a8428f8a5072854fb1df7a8e438970839ab913938ffcb5116261a30c7c78bf
---
...
Deno.test("Notification Concept Scenarios", async (t) => {
  ...
  await t.step("Operational Principle: Configure, Send, and Log Notification", async () => {
    ...
    assertExists(createResult.setting, "Expected setting to be created.");
    const setting: NotificationSetting = createResult.setting!;
    ...
    assertExists(sendResult.sentNotification, "Expected notification to be sent and logged.");
    const sentNotif: SentNotification = sendResult.sentNotification!;
  ...
  await t.step("Scenario: Editing an existing notification configuration", async () => {
    ...
    const settingID = createRes.setting!._id;
    ...
    assertEquals(editRes, {}, "Expected empty result for successful edit.");
  ...
  await t.step("Scenario: Deleting a notification configuration and associated logs", async () => {
    ...
    assertExists(sendRes.sentNotification);
    const sentNotifID = sendRes.sentNotification!._id;
    ...
    assertEquals(deleteRes, {}, "Expected empty result for successful deletion.");
  ...
  await t.step("Scenario: Sending notification with disabled or non-existent configuration", async () => {
    ...
    // 1. Attempt to send with no config for David
    ...
    assertExists(sendNoConfigRes.error, "Expected error when no config exists.");
    assertFalse(!!sendNoConfigRes.sentNotification, "Did not expect notification to be sent without config.");
    ...
    console.log("Output:", createRes);
    assertExists(createRes.setting);
    const disabledSettingID = createRes.setting!._id;
    console.log("Created disabled setting ID:", disabledSettingID);

    // 2. Attempt to send with disabled config
    ...
    assertExists(sendDisabledRes.error, "Expected error when config is disabled.");
    assertFalse(!!sendDisabledRes.sentNotification, "Did not expect notification to be sent with disabled config.");
  ...
  await t.step("Scenario: Precondition failures and error returns for actions", async () => {
    ...
    // Create a valid config for Eve for subsequent tests in this scenario
    ...
    // Test editNotificationConfig with non-existent settingID
    ...
    // Test deleteNotificationConfig with non-existent settingID
    ...
    // Test sendNotification with invalid contextData type
  ...
  await t.step("Scenario: Multiple notification settings for different items/types for one user", async () => {
    ...
    assertExists(createNewsFeed.setting);
    const newsSettingID = createNewsFeed.setting!._id;
    console.log("Created News Feed setting ID:", newsSettingID);

    // Setting 2: Critical Alerts (different item, different type)
    ...
    assertExists(createAlerts.setting);
    const alertSettingID = createAlerts.setting!._id;
    ...
    assertExists(sendNewsResult.sentNotification);
    ...
    assertExists(sendAlertResult.sentNotification);
  ...
  await t.step("Scenario: Querying functionality for notification settings and logs", async () => {
    ...
    // Create a few settings for Grace
    ...
    assertExists(s1Res.setting); assertExists(s2Res.setting); assertExists(s3Res.setting);
    console.log(`Created 3 settings for Grace: S1(${s1Res.setting!._id}, enabled), S2(${s2Res.setting!._id}, enabled), S3(${s3Res.setting!._id}, disabled)`);

    // Send a notification for s1 to create a sent log for query verification
    const sendS1Input = { owner: userGrace, targetItem: itemA, notificationType: "type1", contextData: { key: "value" } };
    await notificationConcept.sendNotification(sendS1Input);
    ...
    console.log("\nQuery: _getSentNotificationsForSetting");
    console.log("Input (settingID S1):", { settingID: s1Res.setting!._id });
    const { sentNotifications: s1Sent } = await notificationConcept._getSentNotificationsForSetting({ settingID: s1Res.setting!._id });
    ...
    console.log("Input (settingID S2):", { settingID: s2Res.setting!._id });
    const { sentNotifications: s2Sent } = await notificationConcept._getSentNotificationsForSetting({ settingID: s2Res.setting!._id });
    ...
    assertEquals(enabledSetting1!._id, s1Res.setting!._id);
