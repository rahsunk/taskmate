---
timestamp: 'Sun Oct 19 2025 18:05:04 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251019_180504.d67a4a38.md]]'
content_id: 16c50bc28ceb91b620e0862c9fab26178ac364978e65b63f830aa2cd96ee7f4c
---

# response:

The TypeScript errors in the `getNextSequence` method indicate a discrepancy in how Deno's TypeScript environment or the specific `npm:mongodb` version interprets the return type of `findOneAndUpdate`.

The standard `findOneAndUpdate` method from the `mongodb` driver typically returns a `Promise<ModifyResult<TDocument>>`, where `ModifyResult` is an object with a `value` property that holds the updated document (or `null` if not found). However, your error messages suggest that `result` is being typed directly as `WithId<CounterDoc> | null`.

If `result` is `WithId<CounterDoc> | null`:

1. `'result' is possibly null`: This means `result` itself could be `null`. Accessing properties on a potentially `null` object requires a null check (e.g., using optional chaining `?.`).
2. `Property 'value' does not exist on type 'WithId< CounterDoc >'`: This confirms that TypeScript believes `result` (when not null) *is* the `CounterDoc` document, not a wrapper object that *contains* the `CounterDoc` in a `value` property. Therefore, you should access `sequence_value` directly on `result`.

The fix involves adjusting the access pattern to directly address `result?.sequence_value`.

Here's the corrected `getNextSequence` method:
