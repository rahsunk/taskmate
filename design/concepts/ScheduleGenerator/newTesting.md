...
Deno.test("ScheduleGenerator Concept Tests", async (t) => {
  ...
  // Cleanup: testDb() typically handles dropping collections before each test run.
  // We just need to close the client after all tests in this file are done.
  ...
  await t.step("Scenario 1: Operational Principle - Basic Schedule Generation", async () => {
    ...
    assert(!getScheduleDetailsResult.error, `_getScheduleDetails failed: ${getScheduleDetailsResult.error}`);
    const fetchedSchedule = getScheduleDetailsResult.scheduleDetails as ScheduleDoc[];
    assertEquals(fetchedSchedule.length, 1);
    assertEquals(fetchedSchedule[0].owner, userA);
    console.log(`  -> Verified schedule details for owner ${fetchedSchedule[0].owner}.`);
    ...
    const fetchedEvents = getEventsResult.event as Event[];
    assertEquals(fetchedEvents.length, 2);
    assert(fetchedEvents.includes(event1Id), "fetchedEvents should include event1Id");
    assert(fetchedEvents.includes(event2Id), "fetchedEvents should include event2Id");
    console.log(`  -> Verified ${fetchedEvents.length} events for schedule ${scheduleId}.`);
    ...
    const fetchedTasks = getTasksResult.task as Task[];
    assertEquals(fetchedTasks.length, 2);
    assert(fetchedTasks.includes(task1Id), "fetchedTasks should include task1Id");
    assert(fetchedTasks.includes(task2Id), "fetchedTasks should include task2Id");
    console.log(`  -> Verified ${fetchedTasks.length} tasks for schedule ${scheduleId}.`);
    ...
    const planEventIds = generatedPlan.filter(item => item.type === "event").map(item => item.originalId);
    assert(planEventIds.includes(event1Id), "Generated plan should include event 1.");
    assert(planEventIds.some(id => id === event2Id), "Generated plan should include event 2."); // Event 2 is a single occurrence, so should be there if within horizon
  ...
  await t.step("Scenario 2: Editing and Regenerating", async () => {
    ...
    assert(!initialGenerateResult.error, `initial generateSchedule failed: ${initialGenerateResult.error}`);
    const initialPlan = initialGenerateResult.generatedPlan as ScheduledItem[];
    assert(initialPlan.some(item => item.originalId === eventId), "Event to Delete should be in initial plan.");
    assert(initialPlan.some(item => item.originalId === taskId), "Task to Delete should be in initial plan.");
    console.log(`  -> Initial schedule generated with items to delete present.`);
  ...
  await t.step("Scenario 4: Error Handling - Precondition Failures", async () => {
    ...
    // This is the variable causing the reported error if missing or mistyped.
    const nonExistentId: ID = "nonExistent:ID" as ID;
  ...
  await t.step("Scenario 5: Querying Data", async () => {
    ...
    assert(!getAllSchedulesResult.error);
    const allSchedules = getAllSchedulesResult.schedule as ScheduleDoc[];
    // Count schedules from previous successful initializations:
    // S1: 1 (for userA)
    // S2: 1 (for userB)
    // S4: 1 (for user:ErrorUser)
    // S5: 2 (for queryUserA, queryUserB)
    // Total: 1 + 1 + 1 + 2 = 5 schedules
    assertEquals(allSchedules.length, 5, "Should return all 5 schedules (User A, User B, ErrorUser, QueryUserA, QueryUserB).");
    assert(allSchedules.some(s => s._id === scheduleIdA), "Schedule A should be in all schedules.");
    assert(allSchedules.some(s => s._id === scheduleIdB), "Schedule B should be in all schedules.");
    console.log(`  -> Retrieved all schedules: ${allSchedules.length} total.`);
    ...
    assert(!getAllEventsResult.error);
    const allEvents = getAllEventsResult.event as EventDoc[];
    // Count events from successful additions:
    // S1: 2 events (event1Id, event2Id)
    // S2: 1 event (eventId)
    // S3: 1 event (remainingEventId, after deletion)
    // S4: 1 event (addEventResult for Long Event)
    // S5: 2 events (eventA1Id, eventB1Id)
    // Total: 2 + 1 + 1 + 1 + 2 = 7 events.
    assertEquals(allEvents.length, 7, "Should return all successfully added events that were not deleted.");
    console.log(`  -> Retrieved all events: ${allEvents.length} total.`);
    ...
    assert(!getAllTasksResult.error);
    const allTasks = getAllTasksResult.task as TaskDoc[];
    // Count tasks from successful additions:
    // S1: 2 tasks (task1Id, task2Id)
    // S2: 1 task (taskId)
    // S3: 1 task (remainingTaskId, after deletion)
    // S4: 1 task (addTaskResult for Urgent Task)
    // S5: 1 task (taskA1Id)
    // Total: 2 + 1 + 1 + 1 + 1 = 6 tasks.
    assertEquals(allTasks.length, 6, "Should return all successfully added tasks that were not deleted.");
    console.log(`  -> Retrieved all tasks: ${allTasks.length} total.`);
  ...
  await t.step("Scenario 6: Schedule Generation Conflict (Edge Case)", async () => {
    ...
    eventStartTime.setHours(9, 0, 0, 0);
    const eventEndTime = new Date(now);
    eventEndTime.setHours(17, 0, 0, 0); // Event takes up most of the day (8 hours)
    ...
    // Generate schedule - this should fail if no free slots are found for the task
    // Daily task scheduling is from 8 AM to 10 PM.
    // Long Event is 9 AM to 5 PM.
    // This leaves 8-9 AM (1 hr) and 5 PM - 10 PM (5 hrs) for tasks. Total 6 hours.
    // Urgent Task requires 3 hours. It *should* be schedulable within these slots.
    // Let's re-evaluate the expectation.
    // The previous prompt said: "If doing this is not possible (e.g., due to conflicts), then return an error."
    // The current implementation attempts to schedule within a 7-day horizon.
    // The task has 3 hours remaining (180 minutes).
    // Available slots for the day: 8-9 AM (60 min), 5-10 PM (300 min).
    // Total available: 360 min. Task needs 180 min. It should fit.
    // The error condition is explicitly "Not all tasks could be scheduled".
    // This scenario should *not* fail if the task can fit.

    // Let's create a scenario where it *actually* cannot be scheduled.
    // Make the task longer, or the available time shorter.

    // Modify the task to require more time than available
    const addTaskResultConflict = await concept.addTask({
        schedule: scheduleId,
        name: "Conflicting Task",
        deadline: taskDeadline, // Same deadline (tomorrow)
        expectedCompletionTime: 400, // Now 6 hours 40 minutes, more than the 6 hours available in a day
        completionLevel: 0,
        priority: 100,
      });
    assert(!addTaskResultConflict.error);
    console.log(`  -> Conflicting task added with ID: ${addTaskResultConflict.task as Task}`);

    // Generate schedule - this *should* fail due to insufficient time
    console.log("Action: generateSchedule - expecting conflict due to insufficient time");
    const generateConflictResult = await concept.generateSchedule({ schedule: scheduleId });
    assert(generateConflictResult.error, "generateSchedule should return an error if tasks cannot be scheduled due to insufficient time.");
    assert(
      generateConflictResult.error?.includes("Not all tasks could be scheduled"),
      `Error message mismatch: ${generateConflictResult.error}`,
    );
    console.log(`  -> generateSchedule failed as expected: ${generateConflictResult.error}`);
...
Deno.test("ScheduleGenerator Concept Tests", async (t) => {
  ...
  // Define some test IDs at the top-level scope of the Deno.test block
  // if they are truly meant to be shared across multiple t.step scenarios.
  // Otherwise, redeclare them within the specific t.step.
  const userA: User = "user:Alice" as ID;
  const userB: User = "user:Bob" as ID;
  ...
  await t.step("Scenario 1: Operational Principle - Basic Schedule Generation", async () => {
    ...
    // 3. Add some tasks with different priorities/deadlines
    const deadline1 = new Date(); // Use new Date() for each scenario for isolation
    deadline1.setDate(today.getDate() + 2); // 2 days from now
    deadline1.setHours(17, 0, 0, 0);

    const deadline2 = new Date(); // Use new Date() for each scenario for isolation
    deadline2.setDate(today.getDate() + 1); // 1 day from now
    deadline2.setHours(12, 0, 0, 0);
  ...
  await t.step("Scenario 3: Deletion and Regenerating", async () => {
    ...
    const taskDeadline1 = new Date(); // Use new Date() for each scenario for isolation
    taskDeadline1.setDate(now.getDate() + 2);
    taskDeadline1.setHours(10, 0, 0, 0);

    const taskDeadline2 = new Date(); // Use new Date() for each scenario for isolation
    taskDeadline2.setDate(now.getDate() + 3);
    taskDeadline2.setHours(10, 0, 0, 0);
  ...
  await t.step("Scenario 5: Querying Data", async () => {
    ...
    // Redeclare nonExistentId for this scope
    const nonExistentId: ID = "nonExistent:ID" as ID;
    ...
    // S2: 1 task (taskId)
    // S3: 1 task (remainingTaskId, after deletion)
    // S4: 1 task (addTaskResult for Urgent Task + addTaskResultConflict for Conflicting Task) = 2 tasks
    // S5: 1 task (taskA1Id)
    // Total: 2 + 1 + 1 + 2 + 1 = 7 tasks.
    // Re-evaluating based on previous Scenario 6 output:
    // Urgent Task was added, Conflicting Task was added.
    // So there are 2 from S6.
    // Total: 2 (S1) + 1 (S2) + 1 (S3, remaining) + 2 (S6) + 1 (S5) = 7 tasks.
    assertEquals(allTasks.length, 7, "Should return all successfully added tasks that were not deleted.");
    console.log(`  -> Retrieved all tasks: ${allTasks.length} total.`);
  ...
  await t.step("Scenario 6: Schedule Generation Conflict (Edge Case)", async () => {
    ...
    const addTaskResultConflict = await concept.addTask({
        ...
        name: "Conflicting Task",
        deadline: taskDeadline, // Same deadline (tomorrow)
        expectedCompletionTime: 400, // Now 6 hours 40 minutes, more than the 6 hours available in a day (6 hours total)
        completionLevel: 0,
        priority: 100,
      });
    assert(!addTaskResultConflict.error);
...
Deno.test("ScheduleGenerator Concept Tests", async (t) => {
  ...
  await t.step("Scenario 2: Editing and Regenerating", async () => {
    ...
    // Initialize a new schedule for User B for this scenario's isolation
    const initResult = await concept.initializeSchedule({ owner: userB });
    assert(!initResult.error, `initializeSchedule failed: ${initResult.error}`);
    const scheduleId = initResult.schedule as Schedule;
    console.log(`  -> Schedule initialized for User B with ID: ${scheduleId}`);
    ...
    const addEventResult = await concept.addEvent({
      schedule: scheduleId,
      name: "Original Event", // Renamed for clarity in this scenario
      startTime: eventStartTime,
      endTime: eventEndTime,
      repeat: { frequency: RepeatFrequency.NONE },
    });
    assert(!addEventResult.error, `addEvent failed: ${addEventResult.error}`);
    const eventId = addEventResult.event as Event;
    console.log(`  -> Event (Original Event) added with ID: ${eventId}`);
    ...
    const addTaskResult = await concept.addTask({
      schedule: scheduleId,
      name: "Original Task", // Renamed for clarity in this scenario
      deadline: taskDeadline,
      expectedCompletionTime: 60,
      completionLevel: 0,
      priority: 50,
    ...
    assert(!addTaskResult.error, `addTask failed: ${addTaskResult.error}`);
    const taskId = addTaskResult.task as Task;
    console.log(`  -> Task (Original Task) added with ID: ${taskId}`);
    ...
    assert(initialPlan.some(item => item.originalId === eventId), "Original Event should be in initial plan.");
    assert(initialPlan.some(item => item.originalId === taskId), "Original Task should be in initial plan.");
    ...
    console.log("Action: editEvent - moving Original Event to 12:00-13:00");
    const editEventResult: Empty | { error: string } = await concept.editEvent({
      schedule: scheduleId,
      oldEvent: eventId,
      name: "Updated Event",
    ...
    // 2. Edit an existing task's priority and completion level
    console.log("Action: editTask - increasing priority and completion of Original Task");
    const editTaskResult: Empty | { error: string } = await concept.editTask({
      schedule: scheduleId,
      oldTask: taskId,
      name: "Updated Task",
  ...
  await t.step("Scenario 3: Deletion and Regenerating", async () => {
    ...
    const initResult = await concept.initializeSchedule({ owner: userA }); // Initialize new schedule for userA for isolation
    assert(!initResult.error, `initializeSchedule failed: ${initResult.error}`);
    const scheduleId = initResult.schedule as Schedule;
    console.log(`  -> Schedule initialized for User A (new schedule) with ID: ${scheduleId}`);
  ...
  await t.step("Scenario 5: Querying Data", async () => {
    ...
    assert(!initResultA.error);
    const scheduleIdA = initResultA.schedule as Schedule;
    console.log(`  -> Schedule A initialized for ${userQueryA}: ${scheduleIdA}`);

    const initResultB = await concept.initializeSchedule({ owner: userQueryB });
    assert(!initResultB.error);
    const scheduleIdB = initResultB.schedule as Schedule;
    console.log(`  -> Schedule B initialized for ${userQueryB}: ${scheduleIdB}`);
    ...
    // --- Test _getScheduleByOwner ---
    console.log("Query: _getScheduleByOwner for queryUserA");
    const getScheduleByOwnerResultA = await concept._getScheduleByOwner({ owner: userQueryA });
    assert(!getScheduleByOwnerResultA.error);
    assertEquals(getScheduleByOwnerResultA.schedule as Schedule, scheduleIdA, "Should retrieve schedule A for owner A.");
    console.log(`  -> Retrieved schedule ${getScheduleByOwnerResultA.schedule} for ${userQueryA}.`);

    console.log("Query: _getScheduleByOwner for non-existent user");
    const getScheduleByOwnerErrorResult = await concept._getScheduleByOwner({ owner: "nonExistentUserX" as ID });
    assert(getScheduleByOwnerErrorResult.error, "Should return error for non-existent owner.");
    console.log(`  -> _getScheduleByOwner for non-existent user failed as expected: ${getScheduleByOwnerErrorResult.error}`);
    ...
    assert(!getAllSchedulesResult.error);
    const allSchedules = getAllSchedulesResult.schedule as ScheduleDoc[];
    // Recounting schedules before S5 starts:
    // S1 (userA): 1 schedule
    // S2 (userB): 1 schedule
    ...
    // In S5:
    // +1 (scheduleIdA for userQueryA)
    // +1 (scheduleIdB for userQueryB)
    // Total at end of S5 = 4 + 2 = 6 schedules
    assertEquals(allSchedules.length, 6, "Should return all 6 schedules accumulated across scenarios.");
    assert(allSchedules.some(s => s._id === scheduleIdA), "Schedule A should be in all schedules.");
    assert(allSchedules.some(s => s._id === scheduleIdB), "Schedule B should be in all schedules.");
    console.log(`  -> Retrieved all schedules: ${allSchedules.length} total.`);
    ...
    assert(!getAllEventsResult.error);
    const allEvents = getAllEventsResult.event as EventDoc[];
    // Recounting events before S5 starts:
    // S1: 2 events (event1Id, event2Id)
    // S2: 1 event (eventId)
    // S3: 1 event (remainingEventId, after deletion)
    // S4: 0 events (only failed adds/edits)
    // Total from S1-S4 = 4 events.
    // In S5:
    // +1 (eventA1Id)
    // +1 (eventB1Id)
    // Total at end of S5 = 4 + 2 = 6 events.
    assertEquals(allEvents.length, 6, "Should return all 6 events accumulated across scenarios.");
    console.log(`  -> Retrieved all events: ${allEvents.length} total.`);
    ...
    assert(!getAllTasksResult.error);
    const allTasks = getAllTasksResult.task as TaskDoc[];
    // Recounting tasks before S5 starts:
    // S1: 2 tasks (task1Id, task2Id)
    // S2: 1 task (taskId)
    // S3: 1 task (remainingTaskId, after deletion)
    // S4: 0 tasks (only failed adds/edits)
    // Total from S1-S4 = 4 tasks.
    // In S5:
    // +1 (taskA1Id)
    // Total at end of S5 = 4 + 1 = 5 tasks.
    assertEquals(allTasks.length, 5, "Should return all 5 tasks accumulated across scenarios.");
    console.log(`  -> Retrieved all tasks: ${allTasks.length} total.`);
  ...
  await t.step("Scenario 6: Schedule Generation Conflict (Edge Case)", async () => {
    ...
    const initResult = await concept.initializeSchedule({ owner: userConflict }); // Use global userConflict
    assert(!initResult.error);
    const scheduleId = initResult.schedule as Schedule;
    console.log(`  -> Schedule initialized for ConflictUser with ID: ${scheduleId}`);
...
Deno.test("ScheduleGenerator Concept Tests", async (t) => {
  ...
  await t.step("Scenario 1: Operational Principle - Basic Schedule Generation", async () => {
    ...
    const now = new Date();
    const event1StartTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 9, 0, 0, 0); // Tomorrow, 9 AM
    const event1EndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 10, 0, 0, 0); // Tomorrow, 10 AM

    const event2StartTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 2, 14, 0, 0, 0); // Day after tomorrow, 2 PM
    const event2EndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 2, 15, 0, 0, 0); // Day after tomorrow, 3 PM
    ...
    // 3. Add some tasks with different priorities/deadlines
    const deadline1 = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 3, 17, 0, 0, 0); // 3 days from now, 5 PM
    const deadline2 = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 2, 12, 0, 0, 0); // 2 days from now, 12 PM
  ...
  await t.step("Scenario 2: Editing and Regenerating", async () => {
    ...
    const now = new Date();
    const eventStartTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 10, 0, 0, 0); // Tomorrow 10 AM
    const eventEndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 11, 0, 0, 0); // Tomorrow 11 AM
    ...
    const taskDeadline = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 3, 17, 0, 0, 0); // 3 days from now 5 PM
    ...
    // 1. Edit an event's time
    const newEventStartTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 12, 0, 0, 0); // Tomorrow 12 PM
    const newEventEndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 13, 0, 0, 0); // Tomorrow 1 PM

    console.log("Action: editEvent - moving Original Event for B to tomorrow 12:00-13:00");
    const editEventResult: Empty | { error: string } = await concept.editEvent({
      schedule: scheduleId,
      oldEvent: eventId,
    ...
    // 2. Edit an existing task's priority and completion level
    console.log("Action: editTask - increasing priority and completion of Original Task for B");
    const editTaskResult: Empty | { error: string } = await concept.editTask({
      schedule: scheduleId,
      oldTask: taskId,
    ...
    const updatedEvent = regeneratedPlan.find(item => item.originalId === eventId && item.type === "event");
    assert(updatedEvent, "Updated event should be in the regenerated plan.");
    assertEquals(updatedEvent.name, "Updated Event for B", "Event name should be updated.");
    assertEquals(updatedEvent.scheduledStartTime.toISOString(), newEventStartTime.toISOString(), "Event start time should be updated.");
    assertEquals(updatedEvent.scheduledEndTime.toISOString(), newEventEndTime.toISOString(), "Event end time should be updated.");
    console.log(`  -> Verified event ${eventId} details updated in the plan.`);
    ...
    const updatedTask = regeneratedPlan.find(item => item.originalId === taskId && item.type === "task");
    assert(updatedTask, "Updated task should be in the regenerated plan.");
    assertEquals(updatedTask.name, "Updated Task for B", "Task name should be updated.");
    assertEquals(fetchedTask.completionLevel, 75, "Task completion level should be updated in state.");
    assertEquals(fetchedTask.priority, 99, "Task priority should be updated in state.");
    console.log(`  -> Verified task ${taskId} details updated in the plan and state.`);
  ...
  await t.step("Scenario 3: Deletion and Regenerating", async () => {
    ...
    const now = new Date();
    const event1StartTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 10, 0, 0, 0);
    const event1EndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 11, 0, 0, 0);

    const event2StartTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 2, 14, 0, 0, 0);
    const event2EndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 2, 15, 0, 0, 0);
    ...
    const taskDeadline1 = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 2, 10, 0, 0, 0); // 2 days from now
    const taskDeadline2 = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 3, 10, 0, 0, 0); // 3 days from now
  ...
  await t.step("Scenario 5: Querying Data", async () => {
    ...
    const now = new Date();
    const eventA1Time = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 10, 0, 0, 0);
    const eventA1EndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 11, 0, 0, 0);
    const addEventResultA1 = await concept.addEvent({
      schedule: scheduleIdA, name: "Event A1", startTime: eventA1Time, endTime: eventA1EndTime, repeat: { frequency: RepeatFrequency.NONE },
    });
    assert(!addEventResultA1.error);
    ...
    const taskA1Deadline = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 17, 0, 0, 0);
    const addTaskResultA1 = await concept.addTask({
      schedule: scheduleIdA, name: "Task A1", deadline: taskA1Deadline, expectedCompletionTime: 60, completionLevel: 0, priority: 70,
    });
    assert(!addTaskResultA1.error);
    ...
    const eventB1Time = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 13, 0, 0, 0);
    const eventB1EndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 14, 0, 0, 0);
    const addEventResultB1 = await concept.addEvent({
      schedule: scheduleIdB, name: "Event B1", startTime: eventB1Time, endTime: eventB1EndTime, repeat: { frequency: RepeatFrequency.NONE },
    });
    assert(!addEventResultB1.error);
    ...
    assertEquals(allSchedules.length, 6, "Should return all 6 schedules accumulated across scenarios.");
    assert(allSchedules.some(s => s.owner === userA), "Schedule for userA (from S1) should be in all schedules.");
    assert(allSchedules.some(s => s.owner === userB), "Schedule for userB (from S2) should be in all schedules.");
    assert(allSchedules.some(s => s.owner === userError), "Schedule for userError (from S4) should be in all schedules.");
    assert(allSchedules.some(s => s.owner === userQueryA), "Schedule for userQueryA (from S5) should be in all schedules.");
    assert(allSchedules.some(s => s.owner === userQueryB), "Schedule for userQueryB (from S5) should be in all schedules.");
    console.log(`  -> Retrieved all schedules: ${allSchedules.length} total.`);
  ...
  await t.step("Scenario 6: Schedule Generation Conflict (Edge Case)", async () => {
    ...
    const now = new Date();
    const eventStartTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 9, 0, 0, 0); // Tomorrow 9 AM
    const eventEndTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 17, 0, 0, 0); // Tomorrow 5 PM (8 hours)
    ...
    const taskDeadline = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 18, 0, 0, 0); // Tomorrow 6 PM (after event)
    ...
    assert(!addTaskResultConflict.error);
    console.log(`  -> Overwhelming task added with ID: ${addTaskResultConflict.task as Task}`);
    ...
    console.log("Action: generateSchedule - expecting conflict due to insufficient total time");
    const generateConflictResult = await concept.generateSchedule({ schedule: scheduleId });
    assert(generateConflictResult.error, "generateSchedule should return an error if tasks cannot be scheduled due to insufficient total time.");
    assert(
      generateConflictResult.error?.includes("Not all tasks could be scheduled"),
      `Error message mismatch: ${generateConflictResult.error}`,
    );
