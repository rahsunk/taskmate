...
const authorized = (
  ...
  return frames.query(Sessioning._getUser, { session: sessionValue }, {
    user: activeUser,
  });
};
...
export const SendFriendRequest: Sync = (
  { request, session, receiver, sender },
) => ({
  when: actions([
    Requesting.request,
...
export const SendFriendRequestResponseSuccess: Sync = (
  ...
  when: actions(
    [Requesting.request, { path: "/api/FriendList/sendFriendRequest" }, {
      request,
    }],
...
export const AcceptFriendRequestResponseSuccess: Sync = ({ request }) => ({
  when: actions(
    [Requesting.request, { path: "/api/FriendList/acceptFriendRequest" }, {
      request,
    }],
...
export const DeclineFriendRequestResponseSuccess: Sync = ({ request }) => ({
  when: actions(
    [Requesting.request, { path: "/api/FriendList/declineFriendRequest" }, {
      request,
    }],
...
export const CancelSentRequestResponseSuccess: Sync = ({ request }) => ({
  when: actions(
    [Requesting.request, { path: "/api/FriendList/cancelSentRequest" }, {
      request,
    }],
...
export const RemoveFriendRequest: Sync = (
  { request, session, friendToRemove, remover },
) => ({
  when: actions([
    Requesting.request,
  ...
  then: actions([FriendList.removeFriend, {
    user1: remover,
    user2: friendToRemove,
  }]),
});
...
export const GetFriendshipDetailsRequest: Sync = (
  { request, session, friendshipId, activeUser, results },
) => ({
  when: actions([
    Requesting.request,
...
export const GetFriendRequestDetailsRequest: Sync = (
  { request, session, requestId, activeUser, results },
) => ({
  when: actions([
    Requesting.request,
