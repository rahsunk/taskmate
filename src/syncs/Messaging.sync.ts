...
export const CreateConversationResponseSuccess: Sync = (
  ...
  when: actions(
    [Requesting.request, { path: "/Messaging/createConversation" }, {
      request,
    }],
...
export const SendMessageRequest: Sync = (
  { request, session, sender, conversationId, content },
) => ({
  when: actions([Requesting.request, {
...
export const GetConversationRequest: Sync = (
  { request, session, activeUser, conversationId, conversation },
) => ({
  when: actions([Requesting.request, {
...
export const GetMessagesInConversationRequest: Sync = (
  { request, session, activeUser, conversationId, conversation, messages },
) => ({
  when: actions([Requesting.request, {
...
export const GetConversationsForUserRequest: Sync = (
  { request, session, activeUser, userToGet, conversations },
) => ({
  when: actions([Requesting.request, {
...
export const GetMessageDetailsRequest: Sync = (
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(Messaging._getMessageDetails, { messageId }, {
      message,
    });
    if (frames.length === 0) return new Frames();
    ...
    frames = await new Frames(...newFrames).query(Messaging._getConversation, {
      conversationId,
    }, { conversation });
