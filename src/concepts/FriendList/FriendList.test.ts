...
Deno.test("FriendsListConcept Tests", async (test) => {
  ...
  await test.step("Operational Principle: Alice and Bob become friends, then Alice removes Bob", async () => {
    ...
    console.log(
      `Action: Alice (${userAlice}) sends friend request to Bob (${userBob})`,
    );
    ...
    assertNotEquals(
      requestId,
      undefined,
      "Friend request ID should be returned.",
    );
    ...
    assertEquals(
      requestsAfterSend.length,
      1,
      "Friend request should exist after sending.",
    );

    // 2. Bob accepts the friend request from Alice
    console.log(
      `Action: Bob (${userBob}) accepts friend request from Alice (${userAlice})`,
    );
    ...
    assertEquals(
      ...
      true,
      "Friend request acceptance should be successful.",
    );
    ...
    assertEquals(
      requestsAfterAccept.length,
      0,
      "Friend request should be deleted after acceptance.",
    );

    const { user1, user2 } = friendsListConcept["canonicalizeUsers"](
      userAlice,
      userBob,
    );
    ...
    console.log(
      `Action: Alice (${userAlice}) removes Bob (${userBob}) from friends.`,
    );
    ...
    assertEquals(
      friendshipsAfterRemove.length,
      0,
      "Friendship should be deleted after removal.",
    );
  ...
  await test.step("Scenario 1: Rejecting a request and re-sending", async () => {
    ...
    console.log(
      `Action: Alice (${userAlice}) sends friend request to Carol (${userCarol})`,
    );
    ...
    console.log(
      `Effect: Request ${sendResult1.request} sent. Result:`,
      sendResult1,
    );

    // Carol declines the request
    console.log(
      `Action: Carol (${userCarol}) declines friend request from Alice (${userAlice})`,
    );
    ...
    assertEquals(
      requestsAfterDecline.length,
      0,
      "Request should be deleted after decline.",
    );

    // Alice tries to send another request (should be allowed now)
    console.log(
      `Action: Alice (${userAlice}) sends friend request to Carol (${userCarol}) again`,
    );
    ...
    console.log(
      `Effect: Second request ${sendResult2.request} sent. Result:`,
      sendResult2,
    );

    // Carol accepts the second request
    console.log(
      `Action: Carol (${userCarol}) accepts second friend request from Alice (${userAlice})`,
    );
    const acceptResult = await friendsListConcept.acceptFriendRequest({
      receiver: userCarol,
      sender: userAlice,
    ...
    assertEquals(
      acceptResult.success,
      true,
      "Second request acceptance should be successful.",
    );
    console.log(`Effect: Carol accepted second request. Result:`, acceptResult);

    // Verify friendship
    const { user1: cUser1, user2: cUser2 } = friendsListConcept
      ["canonicalizeUsers"](userAlice, userCarol);
  ...
  await test.step("Scenario 2: Duplicate requests and already friends", async () => {
    ...
    await friendsListConcept.friendships.deleteMany({
      $or: [{ user1: userAlice, user2: userDavid }, {
        user1: userDavid,
        user2: userAlice,
      }],
    });
    await friendsListConcept.friendRequests.deleteMany({
      $or: [{ sender: userAlice, receiver: userDavid }, {
        sender: userDavid,
        receiver: userAlice,
      }],
    });
    ...
    console.log(
      `Action: Alice (${userAlice}) sends friend request to David (${userDavid})`,
    );
    const sendResult1 = await friendsListConcept.sendFriendRequest({
      sender: userAlice,
      receiver: userDavid,
    ...
    console.log(
      `Action: Alice (${userAlice}) tries to send duplicate request to David (${userDavid})`,
    );
    const sendResult2 = await friendsListConcept.sendFriendRequest({
      sender: userAlice,
      receiver: userDavid,
    ...
    if (!("error" in sendResult2)) { // Asserting type for TS
      throw new Error(
        "sendFriendRequest should have failed due to existing request.",
      );
    }
    assertEquals(
      sendResult2.error,
    ...
    console.log(
      `Effect: Duplicate request failed as expected. Error: ${sendResult2.error}`,
    );

    // David sends request to Alice (should fail - request already exists)
    console.log(
      `Action: David (${userDavid}) tries to send request to Alice (${userAlice}) (reverse direction)`,
    );
    ...
    if (!("error" in sendResult3)) { // Asserting type for TS
      throw new Error(
        "sendFriendRequest should have failed due to existing request (reverse).",
      );
    }
    assertEquals(
      sendResult3.error,
      "A pending friend request already exists between these users.",
      "Reverse request should be rejected.",
    );
    console.log(
      `Effect: Reverse request failed as expected. Error: ${sendResult3.error}`,
    );

    // David accepts request from Alice
    console.log(
      `Action: David (${userDavid}) accepts request from Alice (${userAlice})`,
    );
    const acceptResult = await friendsListConcept.acceptFriendRequest({
      receiver: userDavid,
      sender: userAlice,
    ...
    assertEquals(
      acceptResult.success,
      true,
      "David should successfully accept the request.",
    );
    ...
    console.log(
      `Action: Alice (${userAlice}) tries to send request to David (${userDavid}) (already friends)`,
    );
    ...
    if (!("error" in sendResult4)) { // Asserting type for TS
      throw new Error(
        "sendFriendRequest should have failed because users are already friends.",
      );
    }
    assertEquals(
      sendResult4.error,
      "Users are already friends.",
      "Request to existing friend should be rejected.",
    );
    console.log(
      `Effect: Request to existing friend failed as expected. Error: ${sendResult4.error}`,
    );

    // David tries to accept again (should fail - no request)
    console.log(
      `Action: David (${userDavid}) tries to accept a non-existent request from Alice (${userAlice})`,
    );
    ...
    if (!("error" in acceptResult2)) { // Asserting type for TS
      throw new Error(
        "acceptFriendRequest should have failed because no request exists.",
      );
    }
    assertEquals(
      acceptResult2.error,
      "No pending friend request from sender to receiver.",
      "Accepting non-existent request should fail.",
    );
    console.log(
      `Effect: Accepting non-existent request failed as expected. Error: ${acceptResult2.error}`,
    );
  });
  ...
  await test.step("Scenario 3: Cancelling a request", async () => {
    ...
    await friendsListConcept.friendships.deleteMany({
      $or: [{ user1: userEve, user2: userCarol }, {
        user1: userCarol,
        user2: userEve,
      }],
    });
    await friendsListConcept.friendRequests.deleteMany({
      $or: [{ sender: userEve, receiver: userCarol }, {
        sender: userCarol,
        receiver: userEve,
      }],
    });
    ...
    console.log(
      `Action: Eve (${userEve}) sends friend request to Carol (${userCarol})`,
    );
    const sendResult = await friendsListConcept.sendFriendRequest({
      sender: userEve,
      receiver: userCarol,
    ...
    console.log(
      `Effect: Request ${sendResult.request} sent. Result:`,
      sendResult,
    );

    // Eve cancels the request
    console.log(
      `Action: Eve (${userEve}) cancels sent request to Carol (${userCarol})`,
    );
    ...
    assertEquals(
      ...
      true,
      "Request cancellation should be successful.",
    );
    ...
    assertEquals(
      requestsAfterCancel.length,
      0,
      "Friend request should be deleted after cancellation.",
    );

    // Carol tries to accept (should fail)
    console.log(
      `Action: Carol (${userCarol}) tries to accept a cancelled request from Eve (${userEve})`,
    );
    const acceptResult = await friendsListConcept.acceptFriendRequest({
      receiver: userCarol,
      sender: userEve,
    ...
    if (!("error" in acceptResult)) { // Asserting type for TS
      throw new Error(
        "acceptFriendRequest should have failed because request was cancelled.",
      );
    }
    assertEquals(
      acceptResult.error,
      "No pending friend request from sender to receiver.",
      "Accepting cancelled request should fail.",
    );
    console.log(
      `Effect: Accepting cancelled request failed as expected. Error: ${acceptResult.error}`,
    );
  });
  ...
  await test.step("Scenario 4: Edge cases / Errors", async () => {
    ...
    console.log(
      `Action: Alice (${userAlice}) tries to send friend request to herself`,
    );
    ...
    assertEquals(
      selfRequestResult.error,
      "Cannot send a friend request to yourself.",
      "Sending request to self should fail.",
    );
    console.log(
      `Effect: Request to self failed as expected. Error: ${selfRequestResult.error}`,
    );

    // Removing a non-existent friendship
    console.log(
      `Action: Alice (${userAlice}) tries to remove non-existent friendship with Eve (${userEve})`,
    );
    ...
    if (!("error" in nonExistentRemoveResult)) { // Asserting type for TS
      throw new Error(
        "removeFriend for non-existent friendship should have failed.",
      );
    }
    assertEquals(
      nonExistentRemoveResult.error,
      "No friendship found between these users.",
      "Removing non-existent friendship should fail.",
    );
    console.log(
      `Effect: Removing non-existent friendship failed as expected. Error: ${nonExistentRemoveResult.error}`,
    );

    // Accepting a non-existent request
    console.log(
      `Action: David (${userDavid}) tries to accept non-existent request from Eve (${userEve})`,
    );
    const nonExistentAcceptResult = await friendsListConcept
      .acceptFriendRequest({
        receiver: userDavid,
        sender: userEve,
      });
    if (!("error" in nonExistentAcceptResult)) { // Asserting type for TS
      throw new Error(
        "acceptFriendRequest for non-existent request should have failed.",
      );
    }
    assertEquals(
      nonExistentAcceptResult.error,
      "No pending friend request from sender to receiver.",
      "Accepting non-existent request should fail.",
    );
    console.log(
      `Effect: Accepting non-existent request failed as expected. Error: ${nonExistentAcceptResult.error}`,
    );

    // Declining a non-existent request
    console.log(
      `Action: David (${userDavid}) tries to decline non-existent request from Eve (${userEve})`,
    );
    const nonExistentDeclineResult = await friendsListConcept
    ...
    if (!("error" in nonExistentDeclineResult)) { // Asserting type for TS
      throw new Error(
        "declineFriendRequest for non-existent request should have failed.",
      );
    }
    assertEquals(
      nonExistentDeclineResult.error,
      "No pending friend request from sender to receiver found to decline.",
      "Declining non-existent request should fail.",
    );
    console.log(
      `Effect: Declining non-existent request failed as expected. Error: ${nonExistentDeclineResult.error}`,
    );

    // Cancelling a non-existent request
    console.log(
      `Action: Eve (${userEve}) tries to cancel non-existent request to David (${userDavid})`,
    );
    ...
    if (!("error" in nonExistentCancelResult)) { // Asserting type for TS
      throw new Error(
        "cancelSentRequest for non-existent request should have failed.",
      );
    }
    assertEquals(
      nonExistentCancelResult.error,
      "No pending friend request from sender to receiver found to cancel.",
      "Cancelling non-existent request should fail.",
    );
    console.log(
      `Effect: Cancelling non-existent request failed as expected. Error: ${nonExistentCancelResult.error}`,
    );
  });
  ...
  await test.step("Query Tests", async () => {
    ...
    // Setup some state
    const { request: req1 } = await friendsListConcept.sendFriendRequest({
      sender: userAlice,
      receiver: userBob,
    }) as { request: ID };
    const { request: req2 } = await friendsListConcept.sendFriendRequest({
      sender: userCarol,
      receiver: userDavid,
    }) as { request: ID };
    await friendsListConcept.acceptFriendRequest({
      receiver: userBob,
      sender: userAlice,
    }); // Alice-Bob friendship
    await friendsListConcept.sendFriendRequest({
      sender: userEve,
      receiver: userAlice,
    }); // Eve-Alice pending
    ...
    if ("error" in allFriendships) {
      throw new Error(`Query failed: ${allFriendships.error}`);
    }
    assertEquals(allFriendships.length, 1, "Should have 1 friendship.");
    assertArrayIncludes(allFriendships.map((f) => f.user1), [
      friendsListConcept["canonicalizeUsers"](userAlice, userBob).user1,
    ]);
    ...
    const aliceFriendships = await friendsListConcept._getFriendshipsByUser({
      user: userAlice,
    });
    if ("error" in aliceFriendships) {
      throw new Error(`Query failed: ${aliceFriendships.error}`);
    }
    assertEquals(aliceFriendships.length, 1, "Alice should have 1 friendship.");
    assertArrayIncludes(aliceFriendships.map((f) => f.user1), [
      friendsListConcept["canonicalizeUsers"](userAlice, userBob).user1,
    ]);
    ...
    const davidFriendships = await friendsListConcept._getFriendshipsByUser({
      user: userDavid,
    });
    if ("error" in davidFriendships) {
      throw new Error(`Query failed: ${davidFriendships.error}`);
    }
    assertEquals(
      davidFriendships.length,
      0,
      "David should have 0 friendships.",
    );
    ...
    if ("error" in allRequests) {
      throw new Error(`Query failed: ${allRequests.error}`);
    }
    assertEquals(
      allRequests.length,
      2,
    ...
    const carolSentRequests = await friendsListConcept._getSentFriendRequests({
      sender: userCarol,
    });
    if ("error" in carolSentRequests) {
      throw new Error(`Query failed: ${carolSentRequests.error}`);
    }
    assertEquals(
      carolSentRequests.length,
      1,
      "Carol should have sent 1 request.",
    );
    ...
    const aliceReceivedRequests = await friendsListConcept
      ._getReceivedFriendRequests({ receiver: userAlice });
    if ("error" in aliceReceivedRequests) {
      throw new Error(`Query failed: ${aliceReceivedRequests.error}`);
    }
    assertEquals(
      aliceReceivedRequests.length,
      1,
      "Alice should have received 1 request.",
    );
    assertEquals(aliceReceivedRequests[0].sender, userEve);
    console.log(
      "Result _getReceivedFriendRequests (Alice):",
      aliceReceivedRequests,
    );

    // _getFriendshipDetails (existing)
    const existingFriendship = allFriendships[0];
    console.log(
      `Query: _getFriendshipDetails for ID ${existingFriendship._id}`,
    );
    const friendshipDetails = await friendsListConcept._getFriendshipDetails({
      friendshipId: existingFriendship._id,
    });
    if ("error" in friendshipDetails) {
      throw new Error(`Query failed: ${friendshipDetails.error}`);
    }
    assertEquals(
      friendshipDetails.length,
      1,
      "Should find 1 friendship detail.",
    );
    ...
    console.log(
      `Query: _getFriendshipDetails for non-existent ID ${nonExistentId}`,
    );
    const nonExistentFriendshipDetails = await friendsListConcept
      ._getFriendshipDetails({ friendshipId: nonExistentId });
    if ("error" in nonExistentFriendshipDetails) {
      throw new Error(`Query failed: ${nonExistentFriendshipDetails.error}`);
    }
    assertEquals(
      nonExistentFriendshipDetails.length,
      0,
      "Should find 0 friendship details for non-existent ID.",
    );
    console.log(
      "Result _getFriendshipDetails (non-existent):",
      nonExistentFriendshipDetails,
    );

    // _getFriendRequestDetails (existing)
    const existingRequest = allRequests.find((r) => r.sender === userCarol);
    if (!existingRequest) {
      throw new Error("Could not find Carol's request for testing");
    }
    console.log(
      `Query: _getFriendRequestDetails for ID ${existingRequest._id}`,
    );
    const requestDetails = await friendsListConcept._getFriendRequestDetails({
      requestId: existingRequest._id,
    });
    if ("error" in requestDetails) {
      throw new Error(`Query failed: ${requestDetails.error}`);
    }
    ...
    console.log(
      `Query: _getFriendRequestDetails for non-existent ID ${nonExistentRequestId}`,
    );
    const nonExistentRequestDetails = await friendsListConcept
      ._getFriendRequestDetails({ requestId: nonExistentRequestId });
    if ("error" in nonExistentRequestDetails) {
      throw new Error(`Query failed: ${nonExistentRequestDetails.error}`);
    }
    assertEquals(
      nonExistentRequestDetails.length,
      0,
      "Should find 0 request details for non-existent ID.",
    );
    console.log(
      "Result _getFriendRequestDetails (non-existent):",
      nonExistentRequestDetails,
    );
  });
