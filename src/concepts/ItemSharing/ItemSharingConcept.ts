...
export default class ItemSharingConcept {
  ...
  async makeItemShareable({
    ...
    const existingSharedItem = await this.sharedItems.findOne({
      externalItemID,
    });
    if (existingSharedItem) {
      return {
        error:
          `Item with externalItemID ${externalItemID} is already registered for sharing.`,
      };
    }
  ...
  async shareItemWith({
    ...
    if (existingSharedItem.participants.includes(targetUser)) {
      return {
        error:
          `User ${targetUser} is already a participant for shared item ${sharedItem}.`,
      };
    }
  ...
  async unshareItemWith({
    ...
    if (!existingSharedItem.participants.includes(targetUser)) {
      return {
        error:
          `User ${targetUser} is not a participant for shared item ${sharedItem}.`,
      };
    }
    ...
    if (deletedChangeRequestIds.length > 0) {
      await this.changeRequests.deleteMany({
        _id: { $in: deletedChangeRequestIds },
      });
    }
  ...
  async acceptToCollaborate({
    ...
    if (!existingSharedItem.participants.includes(user)) {
      return {
        error:
          `User ${user} is not a participant for shared item ${sharedItem}.`,
      };
    }
    ...
    if (existingSharedItem.acceptedParticipants.includes(user)) {
      return {
        error:
          `User ${user} has already accepted collaboration for shared item ${sharedItem}.`,
      };
    }
  ...
  async requestChange({
    ...
    if (!existingSharedItem.acceptedParticipants.includes(requester)) {
      return {
        error:
          `User ${requester} has not accepted collaboration for shared item ${sharedItem}.`,
      };
    }
  ...
  async confirmChange({
    ...
    if (existingSharedItem.owner !== owner) {
      return {
        error: `User ${owner} is not the owner of shared item ${sharedItem}.`,
      };
    }

    // Precondition: request exists and is associated with sharedItem.
    const existingChangeRequest = await this.changeRequests.findOne({
      _id: request,
    });
    ...
    if (existingChangeRequest.sharedItemPointer !== sharedItem) {
      return {
        error:
          `Change request ${request} is not for shared item ${sharedItem}.`,
      };
    }
    if (!existingSharedItem.changeRequests.includes(request)) {
      return {
        error:
          `Change request ${request} is not listed in shared item ${sharedItem}'s changeRequests.`,
      };
    }
