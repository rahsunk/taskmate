...
Deno.test("Messaging Concept: Operational Principle", async () => {
  ...
  try {
    ...
    const createResult = await messaging.createConversation({
      user1: alice,
      user2: bob,
    });
    ...
    assert(
      ...
      "Conversation creation should not fail.",
    );
    ...
    const aliceConvos = await messaging._getConversationsForUser({
      user: alice,
    });
    ...
    const sendResult1 = await messaging.sendMessage({
      conversationId,
      sender: alice,
    ...
    if ("error" in sendResult1) {
      throw new Error("Test failed: could not send message.");
    }
    ...
    const sendResult2 = await messaging.sendMessage({
      conversationId,
      sender: bob,
    ...
    if ("error" in sendResult2) {
      throw new Error("Test failed: could not send reply.");
    }
    ...
    const history = await messaging._getMessagesInConversation({
      conversationId,
    });
...
Deno.test("Messaging Concept: Interesting Scenarios and Requirements", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      assert(
        "error" in result,
        "Expected an error when creating a self-conversation.",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      const initialResult = await messaging.createConversation({
        user1: alice,
        user2: bob,
      });
      assert(
        !("error" in initialResult),
        "Initial conversation creation should succeed.",
      );

      console.log(
        "ACTION: Attempt to create the same conversation again (Bob and Alice).",
      );
      const duplicateResult = await messaging.createConversation({
        user1: bob,
        user2: alice,
      });
      ...
      assert(
        ...
        "Expected an error when creating a duplicate conversation.",
      );
      assertEquals(
  ...
  await t.step(
    ...
    async () => {
      ...
      console.log(
        `ACTION: Alice sends a message to a fake conversation ID: ${fakeConversationId}`,
      );
      ...
      assert(
        "error" in result,
        "Expected an error when sending to a non-existent conversation.",
      );
      assert(
  ...
  await t.step(
    ...
    async () => {
      ...
      assert(
        !("error" in createResult),
        "Creating a new conversation should succeed.",
      );
      if ("error" in createResult) {
        throw new Error("Test setup failed: could not create conversation.");
      }
      ...
      assert(
        "error" in result,
        "Expected an error when a non-participant sends a message.",
      );
      assertEquals(
        result.error,
  ...
  await t.step(
    ...
    async () => {
      ...
      const emptyResult = await messaging.sendMessage({
        conversationId,
        sender: alice,
        content: "",
      });
      ...
      const whitespaceResult = await messaging.sendMessage({
        conversationId,
        sender: alice,
        content: "   \t\n ",
      });
      ...
      assert(
        ...
        "Expected an error for whitespace-only content.",
      );
