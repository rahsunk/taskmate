...
interface NotificationSetting {
  _id: ID; // Unique identifier for the setting, maps to concept's NotificationSetting ID
  owner: User;
  email: EmailAddress;
  targetItem: Item;
 ...
 *   a `settingID` of type `NotificationSetting` (references NotificationSetting._id)
 *   a `timestamp` of type `Date`
 *   a `wasSent` of type `Boolean`
 *   an `errorMessage` of type `String` (optional, for logging failures)
 */
interface SentNotification {
  _id: ID; // Unique identifier for the sent notification log entry
...
export default class NotificationConcept {
  ...
  constructor(private readonly db: Db) {
    this.notificationSettings = this.db.collection(
   ...
   * @requires `owner` is a valid ID (existence check is external to this concept),
   *           `email` is a valid format, `dailyTime` is in "HH:MM" format.
  ...
  async createNotificationConfig(
    ...
    if (
      !owner || !email || !targetItem || !notificationType ||
      dailyTime === undefined || emailSubjectTemplate === undefined ||
      emailBodyTemplate === undefined || isEnabled === undefined
    ) {
      return {
        error:
          "Missing or invalid required parameters for notification configuration.",
      };
    }
    if (
      typeof emailSubjectTemplate !== "string" ||
      typeof emailBodyTemplate !== "string" || typeof isEnabled !== "boolean"
    ) {
      return {
        error:
          "Invalid type for emailSubjectTemplate, emailBodyTemplate, or isEnabled.",
      };
    }
    // Check if dailyTime is in HH:MM format
    if (!/^(?:2[0-3]|[01]?[0-9]):(?:[0-5]?[0-9])$/.test(dailyTime)) {
      return {
        error: "Invalid dailyTime format. Expected HH:MM (e.g., '09:00').",
      };
    }
    ...
    const newSetting: NotificationSetting = {
      _id: freshID(), // Auto-generated unique ID for the setting
      owner,
      email,
      targetItem,
      notificationType,
   ...
   *           `email` is valid, `dailyTime` is in "HH:MM" format.
  ...
  async editNotificationConfig(
    ...
    // Basic validation for required fields
    if (
      !settingID || !email || !targetItem || !notificationType ||
      dailyTime === undefined || emailSubjectTemplate === undefined ||
      emailBodyTemplate === undefined || isEnabled === undefined
    ) {
      return {
        error:
          "Missing or invalid required parameters for editing notification configuration.",
      };
    }
    if (
      typeof emailSubjectTemplate !== "string" ||
    ...
    const existingSetting = await this.notificationSettings.findOne({
      _id: settingID,
    });
    if (!existingSetting) {
    ...
    try {
      ...
      if (updateResult.matchedCount === 0) {
        return {
          error:
            `Notification setting with ID ${settingID} not found for update (possible race condition).`,
        };
      }
      ...
      return {
        error: `Failed to edit notification configuration: ${
          e.message || "Unknown error"
        }`,
      };
    }
  ...
  async deleteNotificationConfig(
    ...
    try {
      ...
      // Delete all associated sent notifications (cascading delete)
   ...
   * @requires `owner` exists (as a valid ID string), and there exists an `isEnabled`
   *           `NotificationSetting` entry with the same `owner`, `targetItem`,
   *           and `notificationType`. `contextData` is a valid object for templating.
   * @effects Finds the specified `NotificationSetting`. Uses its `emailSubjectTemplate`,
   *          `emailBodyTemplate`, and `email` to construct and "send" the email (simulated here).
   *          Creates a `SentNotification` entry to record the attempt, `timestamp`, `wasSent`
   *          (true for success, false for failure), and an `errorMessage` if applicable.
   *          Returns the `SentNotification` on success, or an `error` string if no matching/enabled
   *          setting was found or email delivery fails.
   */
  async sendNotification(
    ...
    if (
      !owner || !targetItem || !notificationType || contextData === undefined
    ) {
      return { error: "Missing required parameters for sending notification." };
    }
    if (typeof contextData !== "object" || contextData === null) {
      return {
        error: "Invalid contextData format. Expected a dictionary/object.",
      };
    }

    // Requires: owner exists and there exists an enabled NotificationSetting entry with matching criteria.
    const setting = await this.notificationSettings.findOne({
      owner,
      targetItem,
      notificationType,
      isEnabled: true, // Only send if the setting is enabled
    });

    if (!setting) {
      return {
        error:
          "No matching and enabled notification setting found for this owner, item, and type.",
      };
    }

    const _id = freshID(); // Unique ID for the new log entry
    ...
    try {
      ...
      // Catch any potential errors from the templating or actual send attempt (if implemented)
    ...
    try {
      ...
      return {
        error: `Failed to log sent notification: ${
          e.message || "Unknown error"
        }`,
      };
    }
  ...
  async _getEnabledSetting(
    { owner, targetItem, notificationType }: {
      owner: User;
      targetItem: Item;
      notificationType: string;
    },
