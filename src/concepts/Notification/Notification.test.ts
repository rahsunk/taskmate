...
Deno.test("Notification Concept Scenarios", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      const bodyTemplate =
        "Hi {{userName}}, {{commentAuthor}} commented on your post '{{postTitle}}': \"{{commentContent}}\"";
      ...
      const createResult = await notificationConcept.createNotificationConfig(
        createConfigInput,
      );
      ...
      if ("error" in createResult) {
        ...
        assertFalse(
          true,
          `Failed to create notification config: ${createResult.error}`,
        );
        return;
      } else {
      ...
      assertEquals(
        setting.owner,
        userAlice,
        "Setting owner should match input.",
      );
      assertEquals(
        setting.targetItem,
        itemPost123,
        "Setting targetItem should match input.",
      );
      assertEquals(
        setting.notificationType,
        notificationTypeComment,
        "Setting notificationType should match input.",
      );
      assertEquals(
        setting.emailSubjectTemplate,
        subjectTemplate,
        "Setting subject template should match input.",
      );
      assertEquals(
        setting.isEnabled,
        initialEnabled,
        "Setting isEnabled should match input.",
      );
      ...
      const { settings: fetchedSettings } = await notificationConcept
        ._getNotificationSettingsForUser({ owner: userAlice });
      assertEquals(
        fetchedSettings.length,
        1,
        "Expected one setting for Alice in the database.",
      );
      assertEquals(
        fetchedSettings[0]._id,
        setting._id,
        "Fetched setting ID from DB should match the created one.",
      );
      ...
      const sendResult = await notificationConcept.sendNotification(
      ...
      assertEquals(
        sentNotif.settingID,
        setting._id,
        "Sent notification should reference the correct setting ID.",
      );
      assertEquals(
        sentNotif.wasSent,
        true,
        "Expected email to be marked as sent successfully.",
      );
      assertInstanceOf(
        sentNotif.timestamp,
        Date,
        "Expected timestamp to be a Date object.",
      );
      ...
      const { sentNotifications: loggedNotifications } =
        await notificationConcept._getSentNotificationsForSetting({
          settingID: setting._id,
        });
      console.log("Output:", loggedNotifications);

      assertEquals(
        loggedNotifications.length,
        1,
        "Expected one logged notification for this setting in the database.",
      );
      assertEquals(
        loggedNotifications[0]._id,
        sentNotif._id,
        "Logged notification ID from DB should match the sent one.",
      );
      assertEquals(
        loggedNotifications[0].wasSent,
        true,
        "Logged notification in DB should indicate success.",
      );
      ...
      await client.close();
      console.log(
        `--- Test Teardown: DB client closed for step "${t.name}" ---`,
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      const createInput = {
        ...
        notificationType: initialType,
        dailyTime: "17:00",
        emailSubjectTemplate: initialSubject,
      ...
      if ("error" in createRes) {
        ...
        assertFalse(
          true,
          `Failed to create initial notification config: ${createRes.error}`,
        );
        return;
      } else {
      ...
      const editRes = await notificationConcept.editNotificationConfig(
        editInput,
      );
      ...
      if ("error" in editRes) {
        ...
        assertFalse(
          true,
          `Failed to edit notification config: ${editRes.error}`,
        );
        return;
      }
      ...
      assertEquals(
        editRes,
        {} as Empty,
        "Expected empty result for successful edit.",
      );

      // Verify changes using a query
      console.log("\nQuery: _getNotificationSettingsForUser");
      console.log("Input:", { owner: userBob });
      const { settings: updatedSettings } = await notificationConcept
        ._getNotificationSettingsForUser({ owner: userBob });
      console.log("Output:", updatedSettings);
      assertEquals(
        updatedSettings.length,
        1,
        "Expected one setting for Bob after edit.",
      );
      ...
      assertEquals(
        ...
        updatedType,
        "Notification type should be updated.",
      );
      assertEquals(
        updatedSetting.emailSubjectTemplate,
        updatedSubject,
        "Subject template should be updated.",
      );
      assertEquals(
  ...
  await t.step(
    ...
    async () => {
      ...
      const createInput = {
        ...
        targetItem: itemProduct789,
        notificationType: notificationTypePromo,
        dailyTime: "10:00",
      ...
      if ("error" in createRes) {
        ...
        assertFalse(
          true,
          `Failed to create notification config for deletion test: ${createRes.error}`,
        );
        return;
      } else {
        settingID = createRes.setting._id;
      ...
      if ("error" in sendRes) {
        ...
        assertFalse(
          true,
          `Failed to send notification for deletion test: ${sendRes.error}`,
        );
        return;
      } else {
      ...
      const { settings: initialSettings } = await notificationConcept
        ._getNotificationSettingsForUser({ owner: userCarol });
      assertEquals(
        initialSettings.length,
        1,
        "Expected one setting for Carol initially.",
      );
      const { sentNotifications: initialSentLogs } = await notificationConcept
        ._getSentNotificationsForSetting({ settingID: settingID });
      assertEquals(
        initialSentLogs.length,
        1,
        "Expected one sent log for the setting initially.",
      );
      ...
      const deleteRes = await notificationConcept.deleteNotificationConfig(
        deleteInput,
      );
      ...
      if ("error" in deleteRes) {
        ...
        assertFalse(
          true,
          `Failed to delete notification config: ${deleteRes.error}`,
        );
        return;
      }
      assertEquals(
        deleteRes,
        {} as Empty,
        "Expected empty result for successful deletion.",
      );
      ...
      const { settings: afterDeleteSettings } = await notificationConcept
        ._getNotificationSettingsForUser({ owner: userCarol });
      console.log("Output:", afterDeleteSettings);
      assertEquals(
        afterDeleteSettings.length,
        0,
        "Expected no settings for Carol after deletion.",
      );

      console.log("\nQuery: _getSentNotificationsForSetting (after delete)");
      console.log("Input:", { settingID: settingID });
      const { sentNotifications: afterDeleteSentLogs } =
        await notificationConcept._getSentNotificationsForSetting({
          settingID: settingID,
        });
      console.log("Output:", afterDeleteSentLogs);
      assertEquals(
        afterDeleteSentLogs.length,
        0,
        "Expected no sent notifications for this setting after deletion.",
      );
      console.log(
  ...
  await t.step(
    ...
    async () => {
      ...
      const sendNoConfigInput = {
        owner: userDavid,
        targetItem: itemDocXYZ,
        notificationType: notificationTypeReview,
        contextData: contextData,
      };
      console.log("Input:", sendNoConfigInput);
      const sendNoConfigRes = await notificationConcept.sendNotification(
        sendNoConfigInput,
      );
      ...
      if (!("error" in sendNoConfigRes)) {
        ...
        assertFalse(
          true,
          `Expected error when no config exists, but got success: ${
            JSON.stringify(sendNoConfigRes)
          }`,
        );
        return;
      }
      assertExists(
        sendNoConfigRes.error,
        "Expected error message for no config.",
      );
      ...
      if ("error" in createRes) {
        ...
        assertFalse(
          true,
          `Failed to create disabled notification config: ${createRes.error}`,
        );
        return;
      } else {
      ...
      const sendDisabledRes = await notificationConcept.sendNotification(
        sendDisabledInput,
      );
      ...
      if (!("error" in sendDisabledRes)) {
        ...
        assertFalse(
          true,
          `Expected error when config is disabled, but got success: ${
            JSON.stringify(sendDisabledRes)
          }`,
        );
        return;
      }
      assertExists(
        sendDisabledRes.error,
        "Expected error message for disabled config.",
      );
      ...
      const { sentNotifications: loggedDisabled } = await notificationConcept
        ._getSentNotificationsForSetting({ settingID: disabledSettingID });
      console.log("Output:", loggedDisabled);
      assertEquals(
        loggedDisabled.length,
        0,
        "Expected no sent notifications for a disabled setting.",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      const createMissingOwner = await notificationConcept
        .createNotificationConfig(createMissingOwnerInput);
      console.log("Output:", createMissingOwner);
      if (!("error" in createMissingOwner)) {
        await client.close(); // Ensure client is closed on early exit
        assertFalse(
          true,
          `Expected error for missing owner, but got success: ${
            JSON.stringify(createMissingOwner)
          }`,
        );
        return;
      }
      assertExists(
        createMissingOwner.error,
        "Expected error for missing owner (empty string).",
      );
      ...
      const createInvalidTime = await notificationConcept
        .createNotificationConfig(createInvalidTimeInput);
      console.log("Output:", createInvalidTime);
      if (!("error" in createInvalidTime)) {
        await client.close(); // Ensure client is closed on early exit
        assertFalse(
          true,
          `Expected error for invalid dailyTime format, but got success: ${
            JSON.stringify(createInvalidTime)
          }`,
        );
        return;
      }
      assertExists(
        createInvalidTime.error,
        "Expected error for invalid dailyTime format.",
      );
      ...
      const createInvalidEmail = await notificationConcept
        .createNotificationConfig(createInvalidEmailInput);
      console.log("Output:", createInvalidEmail);
      if (!("error" in createInvalidEmail)) {
        await client.close(); // Ensure client is closed on early exit
        assertFalse(
          true,
          `Expected error for invalid email format, but got success: ${
            JSON.stringify(createInvalidEmail)
          }`,
        );
        return;
      }
      assertExists(
        createInvalidEmail.error,
      ...
      if ("error" in createRes) {
        ...
        assertFalse(
          true,
          `Failed to create valid setting for subsequent tests: ${createRes.error}`,
        );
        return;
      } else {
        settingID = createRes.setting._id;
      ...
      console.log(
        `Created valid setting ID: ${settingID} for subsequent tests.`,
      );
      ...
      const editNonExistent = await notificationConcept.editNotificationConfig(
        editNonExistentInput,
      );
      ...
      if (!("error" in editNonExistent)) {
        ...
        assertFalse(
          true,
          `Expected error for non-existent setting ID during edit, but got success: ${
            JSON.stringify(editNonExistent)
          }`,
        );
        return;
      }
      assertExists(
        editNonExistent.error,
        "Expected error for non-existent setting ID during edit.",
      );
      ...
      const deleteNonExistent = await notificationConcept
        .deleteNotificationConfig(deleteNonExistentInput);
      console.log("Output:", deleteNonExistent);
      if (!("error" in deleteNonExistent)) {
        await client.close(); // Ensure client is closed on early exit
        assertFalse(
          true,
          `Expected error for non-existent setting ID during delete, but got success: ${
            JSON.stringify(deleteNonExistent)
          }`,
        );
        return;
      }
      assertExists(
        deleteNonExistent.error,
        "Expected error for non-existent setting ID during delete.",
      );
      ...
      const sendInvalidContext = await notificationConcept.sendNotification(
        sendInvalidContextInput,
      );
      ...
      if (!("error" in sendInvalidContext)) {
        ...
        assertFalse(
          true,
          `Expected error for invalid contextData type, but got success: ${
            JSON.stringify(sendInvalidContext)
          }`,
        );
        return;
      }
      assertExists(
        sendInvalidContext.error,
        "Expected error for invalid contextData type.",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      const createNewsFeed = await notificationConcept.createNotificationConfig(
        createNewsFeedInput,
      );
      ...
      if ("error" in createNewsFeed) {
        ...
        assertFalse(
          true,
          `Failed to create News Feed setting: ${createNewsFeed.error}`,
        );
        return;
      } else {
      ...
      const createAlertsInput = {
        owner: userFrank,
        email: "frank@example.com",
        targetItem: itemAlerts,
        notificationType: "critical_alert",
        dailyTime: "14:00", // A valid time
      ...
      const createAlerts = await notificationConcept.createNotificationConfig(
        createAlertsInput,
      );
      ...
      if ("error" in createAlerts) {
        ...
        assertFalse(
          true,
          `Failed to create Critical Alert setting: ${createAlerts.error}`,
        );
        return;
      } else {
      ...
      const sendNewsInput = {
        ...
        targetItem: itemNewsFeed,
        notificationType: "daily_news",
        contextData: { date: new Date().toLocaleDateString() },
      };
      console.log("Input:", sendNewsInput);
      const sendNewsResult = await notificationConcept.sendNotification(
        sendNewsInput,
      );
      ...
      if ("error" in sendNewsResult) {
        ...
        assertFalse(
          true,
          `Failed to send Daily News notification: ${sendNewsResult.error}`,
        );
        return;
      }
      assertEquals(
        sendNewsResult.sentNotification.settingID,
        newsSettingID,
        "Daily News sent notification linked to correct setting.",
      );
      ...
      const sendAlertResult = await notificationConcept.sendNotification(
        sendAlertInput,
      );
      ...
      if ("error" in sendAlertResult) {
        ...
        assertFalse(
          true,
          `Failed to send Critical Alert notification: ${sendAlertResult.error}`,
        );
        return;
      }
      assertEquals(
        sendAlertResult.sentNotification.settingID,
        alertSettingID,
        "Critical Alert sent notification linked to correct setting.",
      );
      ...
      const allFrankSent = await db.collection<SentNotification>(
        PREFIX + "sentNotifications",
      ).find({}).toArray();
      console.log(
      ...
      assertEquals(
        allFrankSent.length,
        2,
        "Expected two total sent notifications for Frank.",
      );
      console.log(
  ...
  await t.step(
    ...
    async () => {
      ...
      const s1Res = await notificationConcept.createNotificationConfig({
        ...
        dailyTime: "07:00",
        emailSubjectTemplate: "S1",
        emailBodyTemplate: "B1",
        isEnabled: true,
      });
      const s2Res = await notificationConcept.createNotificationConfig({
        ...
        dailyTime: "08:00",
        emailSubjectTemplate: "S2",
        emailBodyTemplate: "B2",
        isEnabled: true,
      });
      const s3Res = await notificationConcept.createNotificationConfig({
        ...
        email: "grace@example.com",
        targetItem: itemA,
        notificationType: "type3", // Same item as S1, different type
        dailyTime: "09:00",
        emailSubjectTemplate: "S3",
        emailBodyTemplate: "B3",
        isEnabled: false, // Disabled
      });
      ...
      if ("error" in s1Res) {
        await client.close();
        assertFalse(true, `Failed to create s1: ${s1Res.error}`);
        return;
      } else s1Setting = s1Res.setting;
      if ("error" in s2Res) {
        await client.close();
        assertFalse(true, `Failed to create s2: ${s2Res.error}`);
        return;
      } else s2Setting = s2Res.setting;
      if ("error" in s3Res) {
        await client.close();
        assertFalse(true, `Failed to create s3: ${s3Res.error}`);
        return;
      } else s3Setting = s3Res.setting;

      console.log(
        `Created 3 settings for Grace: S1(${s1Setting._id}, enabled), S2(${s2Setting._id}, enabled), S3(${s3Setting._id}, disabled)`,
      );
      ...
      const sendS1Result = await notificationConcept.sendNotification(
        sendS1Input,
      );
      if ("error" in sendS1Result) {
        ...
        assertFalse(
          true,
          `Failed to send notification for s1: ${sendS1Result.error}`,
        );
        return;
      }
      ...
      const { settings: graceSettings } = await notificationConcept
        ._getNotificationSettingsForUser({ owner: userGrace });
      console.log(
      ...
      assertEquals(
        graceSettings.length,
        3,
        "Expected 3 settings for Grace from query.",
      );

      // Test _getSentNotificationsForSetting query
      console.log("\nQuery: _getSentNotificationsForSetting");
      console.log("Input (settingID S1):", { settingID: s1Setting._id });
      const { sentNotifications: s1Sent } = await notificationConcept
        ._getSentNotificationsForSetting({ settingID: s1Setting._id });
      console.log("Output (S1 sent logs):", s1Sent.map((s) => s._id));
      assertEquals(
        s1Sent.length,
        1,
        "Expected 1 sent notification for setting 1 from query.",
      );
      console.log("Input (settingID S2):", { settingID: s2Setting._id });
      const { sentNotifications: s2Sent } = await notificationConcept
        ._getSentNotificationsForSetting({ settingID: s2Setting._id });
      console.log("Output (S2 sent logs):", s2Sent.map((s) => s._id));
      assertEquals(
        s2Sent.length,
        0,
        "Expected 0 sent notifications for setting 2 from query (none sent).",
      );

      // Test _getEnabledSetting query
      console.log("\nQuery: _getEnabledSetting");
      const enabledSetting1Input = {
        owner: userGrace,
        targetItem: itemA,
        notificationType: "type1",
      };
      console.log("Input:", enabledSetting1Input);
      const { setting: enabledSetting1 } = await notificationConcept
        ._getEnabledSetting(enabledSetting1Input);
      console.log(
        "Output:",
        enabledSetting1
          ? { _id: enabledSetting1._id, isEnabled: enabledSetting1.isEnabled }
          : null,
      );
      assertExists(
        enabledSetting1,
        "Expected to find enabled setting 1 via query.",
      );
      assertEquals(enabledSetting1._id, s1Setting._id);

      const enabledSetting3Input = {
        owner: userGrace,
        targetItem: itemA,
        notificationType: "type3",
      };
      console.log("Input:", enabledSetting3Input);
      const { setting: enabledSetting3 } = await notificationConcept
        ._getEnabledSetting(enabledSetting3Input);
      console.log(
        "Output:",
        enabledSetting3
          ? { _id: enabledSetting3._id, isEnabled: enabledSetting3.isEnabled }
          : null,
      );
      assertEquals(
        enabledSetting3,
        null,
        "Did not expect to find disabled setting 3 via _getEnabledSetting query.",
      );

      const nonExistentSettingInput = {
        owner: userGrace,
        targetItem: itemB,
        notificationType: "non_existent_type",
      };
      console.log("Input:", nonExistentSettingInput);
      const { setting: nonExistentSetting } = await notificationConcept
        ._getEnabledSetting(nonExistentSettingInput);
      console.log("Output:", nonExistentSetting);
      assertEquals(
        nonExistentSetting,
        null,
        "Did not expect to find non-existent setting via query.",
      );
